...
from securics.core.utils import clean_pid_files
...
def set_logging(foreground_mode=False, debug_mode=0) -> SecuricsLogger:
    """Set cluster logger.
...
def print_version():
    """Print Securics metadata."""
    from securics.core.cluster import __author__, __licence__, __version__, __securics_name__
    print(f"\n{__securics_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('securics-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('securics-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: SecuricsLogger):
    ...
    logger : SecuricsLogger
        Cluster logger.
    """
    from securics.core.cluster import local_server, master
    from securics.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: SecuricsLogger):
    ...
    from securics.core.cluster import local_server, worker
...
def main():
    """Main function of the securics-clusterd script in charge of starting the cluster process."""
    import securics.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.SECURICS_PATH}/logs/cluster.log'):
        os.chown(f'{common.SECURICS_PATH}/logs/cluster.log', common.securics_uid(), common.securics_gid())
        os.chmod(f'{common.SECURICS_PATH}/logs/cluster.log', 0o660)
    ...
    if cluster_configuration['disabled']:
        sys.exit(0)
    try:
        securics.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    securics.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('securics-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to securics
    if not args.root:
        os.setgid(common.securics_gid())
        os.setuid(common.securics_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('securics-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('securics-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('securics-clusterd', pid)


if __name__ == '__main__':
    import securics.core.cluster.utils as cluster_utils
    from securics.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('securics_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0

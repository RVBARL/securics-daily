...
# Set framework path
path.append(dirname(argv[0]) + '/../framework')  # It is necessary to import Securics package

# Import framework
try:
    import securics.agent
    from securics.agent import upgrade_agents, get_upgrade_result, get_agents
    from securics.core import common
    from securics.core.exception import SecuricsError
    from securics.core.cluster import utils as cluster_utils
except Exception as e:
    print("Error importing 'Securics' package.\n\n{0}\n".format(e))
    exit()
...
def get_script_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--version", type=str, help="Version to upgrade. [Default: latest Securics version]")
...
def list_outdated():
    ...
    agents = securics.agent.get_outdated_agents()
...
async def main():
    ...
    except SecuricsError as securics_err:
        print(f"Error {securics_err.code}: {securics_err.message}")
        if args.debug:
            raise

...
def main():
    """securics-logtest main function."""
    # Parse cmdline args
    ...
    if args.version:
        logging.info('%s', Securics.get_description())
        logging.info('%s', Securics.get_license())
        sys.exit(0)
    ...
    # Initialize securics-logtest component
    w_logtest = SecuricsLogtest(location=args.location)
    logging.info('Starting securics-logtest %s', Securics.get_version_str())
    ...
    while True:
        ...
        try:
            ...
            logging.error('** Securics-logtest error ' + str(error))
            continue
        except ConnectionError:
            logging.error('** Securics-logtest error when connecting with securics-analysisd')
            continue
        ...
        if 'messages' in output.keys():
            ...
            for message in output['messages']:
                if message.startswith("WARNING"):
                    logging.warning('** Securics-Logtest: %s', message)
        ...
        # Show securics-logtest output
        SecuricsLogtest.show_output(output)
...
class SecuricsDeamonProtocol:
    """Encapsulate logic communication aspects between Securics daemons."""

    def __init__(self, version: int = 1, origin_module: str = "securics-logtest", module_name: str = "securics-logtest"):
        """Class constructor.
        ...
        origin_module : str
            Origin source module. Default: "securics-logtest"
        module_name : str
            Source module name. Default: "securics-logtest"
        """
        self.protocol = dict()
    ...
    def wrap(self, command: str, parameters: dict) -> str:
        """Wrap data with Securics daemon protocol information.
    ...
    def unwrap(self, msg: dict) -> dict:
        """Unwrap data from Securics daemon protocol information.
...
class SecuricsSocket:
    """Encapsulate securics-socket communication (header with message size)."""
    ...
    def send(self, msg: str) -> bytes:
        """Send and receive data to securics-socket (header with message size).
...
class SecuricsLogtest:
    """Top level class to interact with securics-logtest feature, part of securics-analysisd."""
    ...
    def __init__(self, location: str = "stdin", log_format: str = "syslog"):
        ...
        """
        self.protocol = SecuricsDeamonProtocol()
        self.socket = SecuricsSocket(LOGTEST_SOCKET)
    ...
    def process_log(self, log, token: str = None, options: str = None) -> dict:
        """Send log event to securics-logtest and receive the outcome.
    ...
    def show_output(output: dict):
        ...
        SecuricsLogtest.show_ossec_logtest_like(output)

    def show_ossec_logtest_like(output: dict):
        """Show securics-logtest output like ossec-logtest.

        Parameters
        ----------
        output : dict
            Securics-logtest outcome.
        """
        ...
        if 'predecoder' in output_data:
            SecuricsLogtest.show_phase_info(output_data['predecoder'], ['timestamp', 'hostname', 'program_name'])
        # Decoding phase
        logging.info('')
        logging.info('**Phase 2: Completed decoding.')
        if 'decoder' in output_data and output_data['decoder']:
            SecuricsLogtest.show_phase_info(output_data['decoder'], ['name', 'parent'])
            if 'data' in output_data:
                SecuricsLogtest.show_phase_info(output_data['data'])
        else:
        ...
        if 'rule' in output_data:
            ...
            SecuricsLogtest.show_phase_info(output_data['rule'], ['id', 'level', 'description', 'groups', 'firedtimes'])
        if output['alert']:
            logging.info('**Alert to be generated.')

    def show_phase_info(phase_data: dict, show_first: list = None, prefix: str = ""):
        """Show securics-logtest processing phase information.
        ...
        for field in sorted(phase_data.keys()):
            if isinstance(phase_data.get(field), dict):
                SecuricsLogtest.show_phase_info(phase_data.pop(field), [], prefix + field + '.')
            else:
...
class Securics:
    def get_install_path() -> str:
        """Get Securics installation path, obtained relative to the path of this file.

        Returns
        -------
        str
            Securics installation path.
        """
        return common.find_securics_path()

    def get_info(field: str) -> str:
        """Get Securics information from securics-control.
        ...
        str
            Field value.
        """
        securics_control = os.path.join(Securics.get_install_path(), "bin", "securics-control")
        securics_env_vars = dict()
        try:
            proc = subprocess.Popen([securics_control, "info"], stdout=subprocess.PIPE)
    ...
    def get_version_str() -> str:
        ...
        str
            Securics version.
        """
        return Securics.get_info('SECURICS_VERSION')

    def get_description() -> str:
        """Get Securics description, contact info and version.

        Returns
        -------
        str
            Securics description.
        """
        return f"Securics {Securics.get_version_str()} - Securics Inc."

    def get_license() -> str:
        """Get Securics License statement.
...
def init_logger(args: argparse.Namespace):
    """Initialize securics-logtest logger.

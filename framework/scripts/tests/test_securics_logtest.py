...
import scripts.securics_logtest as securics_logtest


class SecuricsSocketMock:
    """Auxiliary mock."""
...
class SecuricsDeamonProtocolMock:
    """Auxiliary mock."""
...
def test_init_argparse(argument_parser_mock):
    ...
    securics_logtest.init_argparse()
...
@patch('scripts.securics_logtest.init_logger')
@patch('scripts.securics_logtest.SecuricsLogtest')
@patch('scripts.securics_logtest.init_argparse')
@patch('builtins.input', return_value='mock')
def test_main(input_mock, argparse_mock, securics_logtest_class_mock, init_logger_mock, register_mock, logger_warning_mock,
    ...
    securics_logtest_class_mock.return_value = SecuricsLogtestMock()

    # Test the first 'try' present in the 'while'
    try:
        securics_logtest.main()
    ...
    register_mock.assert_called_once_with(securics_logtest_class_mock.return_value.remove_last_session)
    sys_exit_mock.assert_has_calls([call(0), call(1)])

    logger_info_mock.assert_has_calls([call('%s', 'Securics ERROR - Securics Inc.'),
                                       call('%s', '\nThis program is free software; you can redistribute it and/or '
                                                  'modify\nit under the terms of the GNU General Public License '
                                                  '(version 2) as\npublished by the Free Software Foundation. For more '
                                                  'details, go to\nhttps://www.gnu.org/licenses/gpl.html\n'),
                                       call('Starting securics-logtest %s', 'ERROR'),
                                       call('Type one log per line')])
    logger_error_mock.assert_called_once_with('Unit test configuration wrong syntax: %s',
                                              securics_logtest_class_mock.return_value.get_last_ut())
    logger_warning_mock.assert_has_calls([call('** Securics-Logtest: %s', 'WARNING'), call('')])

    assert securics_logtest_class_mock.return_value.show_last_ut_result_called is True
    assert securics_logtest_class_mock.return_value.remove_last_session_called is False
    assert securics_logtest_class_mock.return_value.get_last_ut_called is True
    assert securics_logtest_class_mock.return_value.process_log_called is True
    ...
    securics_logtest_class_mock.return_value = SecuricsLogtestMock(True)
    ...
    securics_logtest_class_mock.return_value = SecuricsLogtestMock(process_log_exception='ValueError')
    ...
    logger_error_mock.assert_called_once_with('** Securics-logtest error ')

    # Test the third exception
    logger_error_mock.reset_mock()
    securics_logtest_class_mock.return_value = SecuricsLogtestMock(process_log_exception='ConnectionError')
    logger_error_mock.side_effect = Exception()
    ...
    logger_error_mock.assert_called_once_with('** Securics-logtest error when connecting with securics-analysisd')


# Test SecuricsDaemonProtocol class methods

def create_securics_daemon_protocol_class():
    """Create new SecuricsDaemonProtocol class."""
    return securics_logtest.SecuricsDeamonProtocol()


def test_wdp_init():
    """Test the init method, checking the initial status of its attributes."""
    wdp = create_securics_daemon_protocol_class()

    assert isinstance(wdp.protocol, dict)
    assert wdp.protocol['version'] == 1
    assert isinstance(wdp.protocol['origin'], dict)
    assert wdp.protocol['origin']['name'] == 'securics-logtest'
    assert wdp.protocol['origin']['module'] == 'securics-logtest'


@patch('json.dumps', return_value='')
def test_wdp_wrap(json_dumps_mock):
    """Test if the data is being properly wrapped with securics daemon protocol information."""
    wdp = create_securics_daemon_protocol_class()
    assert wdp.wrap(command='command', parameters={'parameters': 'parameters'}) == json_dumps_mock.return_value
    json_dumps_mock.assert_called_once_with({'version': 1, 'origin': {'name': 'securics-logtest', 'module':
        'securics-logtest'}, 'command': 'command', 'parameters':
                                                 {'parameters': 'parameters'}})


@patch('json.loads', return_value={'error': 'error', 'message': 'message'})
def test_wdp_unwrap(json_loads_mock):
    """Test if the data is being properly unwrapped from the securics daemon protocol."""
    wdp = create_securics_daemon_protocol_class()
    msg = {}
...
# Test SecuricsSocket methods

def create_securics_socket_class(file):
    """Create a new SecuricsSocket class."""
    return securics_logtest.SecuricsSocket(file=file)


def test_ws_init():
    """Test the correct start of the SecuricsSocket class."""
    file = ''
    assert create_securics_socket_class(file).file == file
...
def test_ws_sent(unpack_mock, pack_mock, msg_waitall_mock, stream_mock, unix_mock, socket_socket_mock):
    ...
    ws = create_securics_socket_class(file=file)
...
# Test SecuricsLogtest class methods

@patch('scripts.securics_logtest.SecuricsSocket', return_value=SecuricsSocketMock())
@patch('scripts.securics_logtest.LOGTEST_SOCKET')
@patch('scripts.securics_logtest.SecuricsDeamonProtocol', return_value=SecuricsDeamonProtocolMock())
def create_securics_logtest_class(securics_deamon_mock, logtest_socket_mock, securics_socket_mock):
    """Create new SecuricsLogtest class."""
    return securics_logtest.SecuricsLogtest()


def test_wl_init():
    """Test the correct initialization of SecuricsLogtest class."""
    wl = create_securics_logtest_class()

    assert isinstance(wl.protocol, SecuricsDeamonProtocolMock)
    assert isinstance(wl.socket, SecuricsSocketMock)
...
@patch('scripts.securics_logtest.SecuricsLogtest.remove_session')
...
@patch('logging.debug')
@patch('json.dumps', return_value='')
@patch('scripts.securics_logtest.SecuricsLogtest.show_ossec_logtest_like')
...
@patch('logging.info')
@patch('scripts.securics_logtest.SecuricsLogtest.show_phase_info')
def test_wl_show_ossec_logtest_like(show_phase_info_mock, info_mock):
    """Test if securics-logtest output is being shown as ossec-logtest output."""
    ...
    # Test the third 'if'
    securics_logtest.SecuricsLogtest.show_ossec_logtest_like(output)
...
def test_wl_show_phase_info(info_mock):
    """Check if securics-logtest is processing phase information."""
    phase_data = {'key': 'value', 'key2': 'value2', 'key3': {'1': '2'}}
    show_first = ['key']

    securics_logtest.SecuricsLogtest.show_phase_info(phase_data=phase_data, show_first=show_first)
    info_mock.assert_has_calls([call("\t%s: '%s'", 'key', 'value'), call("\t%s: '%s'", 'key2', 'value2')])


@patch('logging.info')
@patch('scripts.securics_logtest.SecuricsLogtest.get_last_ut', return_value='mock')
...
def create_securics_class():
    """Auxiliary function to create a Securics class."""
    return securics_logtest.Securics


@patch('scripts.securics_logtest.common.find_securics_path', return_value='')
def test_securics_get_install_path(find_securics_path_mock):
    """Test is we can get the installation path correctly."""
    assert create_securics_class().get_install_path() == find_securics_path_mock.return_value


@patch('subprocess.PIPE')
@patch('subprocess.Popen')
@patch('os.path.join', return_value='')
@patch('scripts.securics_logtest.Securics.get_install_path', return_value='')
def test_securics_get_info(get_install_mock, join_mock, popen_mock, pipe_mock):
    """Check if we can properly obtain information from securics-control."""
    ...
    securics = create_securics_class()
    popen_mock.return_value = ProcMock()
    field = 'key'

    # Test the 'try'
    assert securics.get_info(field=field) == field
    get_install_mock.assert_called_once_with()
    join_mock.assert_called_once_with(get_install_mock.return_value, 'bin', 'securics-control')
    ...
    assert securics.get_info(field=field) == 'ERROR'


@patch('scripts.securics_logtest.Securics.get_info', return_value='')
def test_securics_get_version_str(get_info_mock):
    """Test if the version is being properly retrieved."""
    assert create_securics_class().get_version_str() == get_info_mock.return_value
    get_info_mock.assert_called_once_with('SECURICS_VERSION')


@patch('scripts.securics_logtest.Securics.get_version_str', return_value='')
def test_securics_get_description(get_version_mock):
    """Test if the description is being properly retrieved."""
    assert create_securics_class().get_description() == f"Securics {get_version_mock.return_value} - Securics Inc."
    get_version_mock.assert_called_once_with()


@patch('textwrap.dedent', return_value='')
def test_securics_get_license(dedent_mock):
    """Test that the license was not changed."""
    assert create_securics_class().get_license() == dedent_mock.return_value
...
def test_init_logger(logging_mock):
    ...
    securics_logtest.init_logger(ArgsMock())

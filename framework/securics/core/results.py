...
import securics.core.exception as wexception
...
class AbstractSecuricsResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.SecuricsInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractSecuricsResult):
            self.dikt = dct.dikt
        else:
            raise wexception.SecuricsInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractSecuricsResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractSecuricsResult objects. When merged with a SecuricsException, the result is
        always a SecuricsException

        Parameters
        ----------
        other : AbstractSecuricsResult or wexception.SecuricsException

        Raises
        ------
        SecuricsInternalError(1000)
            Securics internal error when two object could not be merged.

        Returns
        -------
        AbstractSecuricsResult or wexception.SecuricsException
            Resultant object.
        """
        if isinstance(other, wexception.SecuricsException):
            return other
        elif not isinstance(other, (dict, AbstractSecuricsResult)):
            raise wexception.SecuricsInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractSecuricsResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in SecuricsResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in securics.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractSecuricsResult
            Filtered AbstractSecuricsResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in SecuricsResult subclasses.
...
class SecuricsResult(AbstractSecuricsResult):
    """
...
class AffectedItemsSecuricsResult(AbstractSecuricsResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.SecuricsException = None):
        ...
        error : wexception.SecuricsException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsSecuricsResult
        ...
        wexception.SecuricsInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsSecuricsResult):
            raise wexception.SecuricsInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsSecuricsResult with self.

        Parameters
        ----------
        other : AffectedItemsSecuricsResult
            AffectedItemsSecuricsResult object to merge.
        ...
        if isinstance(other, wexception.SecuricsError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.SecuricsException):
            return result
        elif not isinstance(other, AffectedItemsSecuricsResult):
            raise wexception.SecuricsInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsSecuricsResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractSecuricsResult
            Instance of AbstractSecuricsResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsSecuricsResult object.

        Returns
        -------
        dict
            Return AffectedItemsSecuricsResult as a dictionary with fields data, message and error.
        """

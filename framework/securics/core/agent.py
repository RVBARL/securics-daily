...
from securics.core import common, configuration, stats
from securics.core.InputValidator import InputValidator
from securics.core.cluster.utils import get_manager_status
from securics.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from securics.core.exception import SecuricsException, SecuricsError, SecuricsInternalError, SecuricsResourceNotFound
from securics.core.utils import SecuricsVersion, plain_dict_to_nested_dict, get_fields_to_nest, SecuricsDBQuery, \
    SecuricsDBQueryDistinct, SecuricsDBQueryGroupBy, SecuricsDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from securics.core.securics_queue import SecuricsQueue
from securics.core.securics_socket import SecuricsSocket, SecuricsSocketJSON, create_securics_socket_message
...
class SecuricsDBQueryAgents(SecuricsDBQuery):
    """Class used to query Securics agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_securics_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = SecuricsDBBackend(query_format='global')
        SecuricsDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the Securics query."""
        SecuricsDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return SecuricsDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the Securics query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            SecuricsDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise SecuricsError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            SecuricsDBQuery._process_filter(self, field_name, field_filter, q_filter)


class SecuricsDBQueryGroup(SecuricsDBQuery):
    """Class used to query Securics groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = SecuricsDBBackend(query_format='global')
        SecuricsDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class SecuricsDBQueryDistinctAgents(SecuricsDBQueryDistinct, SecuricsDBQueryAgents):
    pass


class SecuricsDBQueryGroupByAgents(SecuricsDBQueryGroupBy, SecuricsDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        SecuricsDBQueryAgents.__init__(self, *args, **kwargs)
        SecuricsDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return SecuricsDBQuery._format_data_into_dictionary(self)


class SecuricsDBQueryMultigroups(SecuricsDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        SecuricsDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        SecuricsResourceNotFound(1701)
            Agent does not exist.
        """
        with SecuricsDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
    ...
    def reconnect(self, wq: SecuricsQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : SecuricsQueue
            SecuricsQueue used for the active response message.
        ...
        # Send force reconnect message to the SecuricsQueue
        ret_msg = wq.send_msg_to_agent(SecuricsQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by Securics.
        """
        # Check that securics-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except SecuricsInternalError as e:
            # securics-authd is not running due to a problem with /proc availability
            raise SecuricsError(1726, extra_message=str(e))

        if manager_status.get('securics-authd') != 'running':
            # securics-authd is not running
            raise SecuricsError(1726)
        ...
        try:
            ...
            raise SecuricsInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = SecuricsSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise SecuricsError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise SecuricsInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to Securics using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise SecuricsError(1705, extra_message=name)
            elif e.code == 9007:
                raise SecuricsError(1706, extra_message=ip)
            elif e.code == 9012:
                raise SecuricsError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = SecuricsDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = SecuricsDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise SecuricsResourceNotFound(1710)
        ...
        if group_id not in group_list:
            raise SecuricsError(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise SecuricsError(1745)
            else:
                set_default = True
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or Securics vX.Y.Z.
        ...
        """
        if SecuricsVersion(agent_version) < SecuricsVersion(common.ACTIVE_CONFIG_VERSION):
            raise SecuricsInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        SecuricsInternalError(1015)
            Agent version is null.
        SecuricsInternalError(1735)
        ...
        if self.version is None:
            raise SecuricsInternalError(1015)
        agent_version = SecuricsVersion(self.version.split(" ")[1])
        required_version = SecuricsVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise SecuricsInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_securics_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_securics_version_format(filters: dict):
    """Verify and format the specified securics version into the 'securics vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"securics {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^securics \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: SecuricsQueue = None) -> str:
    ...
    wq : SecuricsQueue
        SecuricsQueue used for the active response messages.
    ...
    str
        Message generated by Securics.
    """
    # If the Securics agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if SecuricsVersion(agent_version) >= SecuricsVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(SecuricsQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(SecuricsQueue.RESTART_AGENTS, agent_id)
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_securics_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_securics_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = SecuricsSocket(common.UPGRADE_SOCKET)

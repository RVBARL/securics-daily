...
def load_decoders_from_file(decoder_file: str, decoder_path: str, decoder_status: str) -> list:
    ...
    try:
        decoders = list()
        position = 0
        root = load_securics_xml(os.path.join(common.SECURICS_PATH, decoder_path, decoder_file))
        ...
        raise SecuricsError(1502, extra_message=os.path.join('SECURICS_HOME', decoder_path, decoder_file))
    except Exception:
        raise SecuricsInternalError(1501, extra_message=os.path.join('SECURICS_HOME', decoder_path, decoder_file))

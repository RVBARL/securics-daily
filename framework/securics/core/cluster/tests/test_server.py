...
with patch('rvbionics.common.securics_uid'):
    with patch('rvbionics.common.securics_gid'):
        from securics.core.cluster.server import *
        from securics.core.cluster import common as c_common
        from securics.core.exception import SecuricsClusterError, SecuricsError, SecuricsResourceNotFound
...
async def test_AbstractServerHandler_init(event_loop):
    ...
    with patch("securics.core.cluster.server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("securics.core.cluster.server.AbstractServerHandler.hello")
@patch("securics.core.cluster.server.AbstractServerHandler.echo_master")
@patch("securics.core.cluster.common.Handler.process_request")
async def test_AbstractServerHandler_process_request(mock_process_request, mock_echo_master, mock_hello, event_loop):
    ...
    abstract_server_handler.process_request(command=b"echo-c", data=b"securics")
    mock_echo_master.assert_called_once_with(b"securics")
...
async def test_AbstractServerHandler_echo_master(event_loop):
    ...
    assert abstract_server_handler.echo_master(data=b"securics") == (b"ok-m ", b"securics")
    abstract_server_handler.echo_master(data=b"securics")
...
def test_AbstractServerHandler_hello(event_loop):
    ...
    with pytest.raises(SecuricsClusterError, match=".* 3029 .*"):
        abstract_server_handler.hello(b"elif_test")

    abstract_server_handler.server.clients["if_test"] = "testing"
    with pytest.raises(SecuricsClusterError, match=f".* 3028 .* if_test"):
        abstract_server_handler.hello(b"if_test")
    assert abstract_server_handler.name == ""


@pytest.mark.asyncio
@patch("securics.core.cluster.common.Handler.process_response")
...
@pytest.mark.asyncio
@patch("asyncio.Queue")
@patch("securics.core.cluster.server.functools")
...
@patch('securics.core.cluster.server.AbstractServer.check_clients_keepalive')
@patch('securics.core.cluster.server.AbstractServerHandler')
...
@patch("securics.core.cluster.server.uuid4", return_value="abc123")
@patch("asyncio.get_running_loop", new=Mock())
...
@patch("securics.core.cluster.server.utils.process_array")
@patch("asyncio.get_running_loop", new=Mock())
def test_AbstractServer_get_connected_nodes(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        abstract_server.get_connected_nodes(search={"value": "securics", "negation": True},
                                            sort={"fields": ["nothing"], "order": "desc"}, limit=501, offset=1)
        mock_process_array.assert_called_once_with([basic_dict["info"]], search_text="securics", complementary_search=True,
...
def test_AbstractServer_get_connected_nodes_ko(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        ...
        with pytest.raises(SecuricsError, match=".* 1724 .* Not a valid select field: no"):
            abstract_server.get_connected_nodes(select={"no": "exists"})

        with pytest.raises(SecuricsError, match=".* 1728 .*"):
            abstract_server.get_connected_nodes(filter_type="None")

        abstract_server.configuration = {"node_name": "master"}
        with pytest.raises(SecuricsResourceNotFound, match=".* 1730 .*"):
...
async def test_AbstractServer_check_clients_keepalive(sleep_mock):
    ...
    with patch.object(logger, "debug") as mock_debug:
        with patch.object(logger, "error") as mock_error:
            with patch("securics.core.cluster.server.AbstractServer.setup_task_logger",
                       return_value=logger):
                abstract_server = AbstractServer(performance_test=1, concurrency_test=2, configuration={"test3": 3},
                                                 cluster_items={"test4": 4}, enable_ssl=True, logger=logger)
...
@patch("asyncio.sleep", side_effect=IndexError)
@patch("asyncio.get_running_loop", new=Mock())
@patch('securics.core.cluster.server.perf_counter', return_value=0)

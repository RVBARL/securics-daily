...
import securics.core.cluster.cluster
import securics.core.cluster.utils
import securics.core.manager
import securics.core.results as wresults
from securics import agent
from securics.cluster import get_node_wrapper, get_nodes_info
from securics.core import common, exception
from securics.core.cluster import local_client, common as c_common
from securics.core.cluster.cluster import check_cluster_status
from securics.core.exception import SecuricsException, SecuricsClusterError, SecuricsError
from securics.core.securics_socket import securics_sendsync
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = securics.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        self.node_info = securics.core.cluster.cluster.get_node() if node is None else node.get_node()
        ...
        if not basic_services:
            self.basic_services = ('securics-modulesd', 'securics-analysisd', 'securics-execd', 'securics-db', 'securics-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'securics-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.SecuricsException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or SecuricsException
            Dictionary with API response or SecuricsException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_securics_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractSecuricsResult, exception.SecuricsException)) \
                else wresults.SecuricsResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.SecuricsInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.SecuricsInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.SecuricsClusterError))
            return e
        except exception.SecuricsError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.SecuricsInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_securics_status(self):
        """
        There are some services that are required for securics to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * Securics must be started before using the API is the services are stopped.

        The basic services securics needs to be running are: securics-modulesd, securics-remoted, securics-analysisd, securics-execd
        and securics-db
        """
        if self.f == securics.core.manager.status:
            return

        status = securics.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.SecuricsError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.SecuricsInternalError(3021)
                except OperationalError as exc:
                    raise exception.SecuricsInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.SecuricsInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.SecuricsInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.SecuricsInternalError(900)
            ...
            # SecuricsClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913} and not isinstance(e,
                                                                                           exception.SecuricsClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.SecuricsJSONEncoder)
        except (exception.SecuricsError, exception.SecuricsResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.SecuricsJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.SecuricsInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.SecuricsJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.SecuricsException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.SecuricsException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of SecuricsInternalError
        if isinstance(e, exception.SecuricsInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('SECURICS_HOME', os.path.relpath(h.baseFilename, start=common.SECURICS_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        res = json.loads(await client.send_file(os.path.join(common.SECURICS_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_securics_object)
        os.remove(os.path.join(common.SECURICS_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.SecuricsJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_securics_object)

    async def forward_request(self) -> [wresults.AbstractSecuricsResult, exception.SecuricsException]:
        ...
        Returns
        -------
        wresults.AbstractSecuricsResult or exception.SecuricsException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractSecuricsResult, exception.SecuricsException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a SecuricsError exception
                if isinstance(result, SecuricsClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = SecuricsError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractSecuricsResult, exception.SecuricsException)) \
                else wresults.SecuricsResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsSecuricsResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractSecuricsResult):
        ...
        # It might be a SecuricsError after reducing
        if isinstance(response, wresults.AffectedItemsSecuricsResult):
...
class SecuricsRequestQueue:
    """Represents a queue of Securics requests"""
...
class APIRequestQueue(SecuricsRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('securics').getChild('dapi')
        self.logger.addFilter(securics.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_securics_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.SecuricsJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(SecuricsRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('securics').getChild('sendsync')
        self.logger.addFilter(securics.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await securics_sendsync(**request)

...
from securics.core.cluster import client, cluster, server
from securics.core.cluster import common as c_common
from securics.core.cluster.dapi import dapi
from securics.core.cluster.utils import context_tag
...
class LocalServer(server.AbstractServer):
    ...
    async def start(self):
        ...
        socket_path = os.path.join(common.SECURICS_PATH, 'queue', 'cluster', 'c-internal.sock')
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            ...
            if node_name in self.server.node.clients:
                ...
                raise SecuricsClusterError(3022)
        else:
            return super().process_request(command, data)
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            if self.server.node.client is None:
                raise SecuricsClusterError(3023)
            asyncio.create_task(self.log_exceptions(

...
from securics.core.exception import SecuricsError, SecuricsException, SecuricsInternalError, SecuricsHAPHelperError
from securics.core.results import SecuricsResult
from securics.core.utils import temporary_cache
from securics.core.securics_socket import create_securics_socket_message
from securics.core.wlogging import SecuricsLogger
...
execq_lockfile = os.path.join(common.SECURICS_PATH, "var", "run", ".api_execq_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'securics_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise SecuricsError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise SecuricsError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise SecuricsHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    ...
    try:
        ...
        if e.code == 1106:
            ...
            raise SecuricsError(3006, extra_message=e.message)
    except Exception as e:
        raise SecuricsError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise SecuricsError(3004, extra_message="Cluster port must be an integer.")
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    try:
        ...
        raise SecuricsInternalError(1913, extra_message=str(e))

    processes = ['securics-agentlessd', 'securics-analysisd', 'securics-authd', 'securics-csyslogd', 'securics-dbd', 'securics-monitord',
                 'securics-execd', 'securics-integratord', 'securics-logcollector', 'securics-maild', 'securics-remoted',
                 'securics-reportd', 'securics-syscheckd', 'securics-clusterd', 'securics-modulesd', 'securics-db', 'securics-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.SECURICS_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    try:
        cluster_status |= {"running": "yes" if get_manager_status()['securics-clusterd'] == 'running' else "no"}
...
def manager_restart() -> SecuricsResult:
    """Restart Securics manager.

    Send JSON message with the 'restart-securics' command to common.EXECQ_SOCKET socket.

    Raises
    ------
    SecuricsInternalError(1901)
        If the socket path doesn't exist.
    SecuricsInternalError(1902)
    ...
    try:
        ...
        # json msg for restarting Securics manager
        msg = json.dumps(create_securics_socket_message(origin={'module': common.origin_module.get()},
                                                     command=common.RESTART_SECURICS_COMMAND,
        ...
        if os.path.exists(socket_path):
            try:
                ...
                raise SecuricsInternalError(1902)
        else:
    ...
    return SecuricsResult({'message': 'Restart request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.SECURICS_PATH, here, 'cluster.json')) as f:
        ...
        raise SecuricsError(3005, str(e))
...
class ClusterLogger(SecuricsLogger):
    """
    Define the logger used by securics-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from securics-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'securics-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `SecuricsResult` instance in case the execution did not fail. Return an exception otherwise.
    """

    import concurrent
    from asyncio import run

    from securics.core.cluster.dapi.dapi import DistributedAPI

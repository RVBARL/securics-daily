...
from securics.tests.test_security import db_setup  # noqa
...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a SecuricsResult.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    with patch('securics.core.security.change_keypair', side_effect=None):
        security, SecuricsResult, _ = db_setup
        mock_current_user = ContextVar('current_user', default='securics')
        with patch("securics.sca.common.current_user", new=mock_current_user):
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('securics.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('securics.core.security.TokenManager.add_user_roles_rules')
...
@patch("securics.core.security.revoke_tokens")
@patch("securics.core.security.check_database_integrity")
@patch("securics.core.security.os.remove")

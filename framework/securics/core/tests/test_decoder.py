...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        ...
        from securics.core import decoder
...
@pytest.mark.parametrize("detail, value, details, expected_result", [
    ("parent", "securics", {'regex': ['regex00']}, {'parent': 'securics', 'regex': ['regex00']}),
...
@pytest.mark.parametrize("filename, relative_dirname, status, permissions, exception", [
    ...
    ('wrong_decoders.xml', 'decoders', "all", 777, SecuricsInternalError(1501)),
    ('non_existing.xml', 'decoders', "disabled", 777, SecuricsError(1502)),
    ('test1_decoders.xml', 'decoders', "all", 000, SecuricsError(1502)),
])
@patch('securics.core.common.SECURICS_PATH', new=test_data_path)

...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        from securics.core.manager import *
...
@patch('securics.core.cluster.utils.glob')
...
def test_get_ossec_log_fields():
    ...
    assert result[1] == 'securics-rootcheck'
...
def test_get_ossec_logs(log_format):
    ...
    with patch("securics.core.manager.get_securics_active_logging_format", return_value=log_format):
        with pytest.raises(SecuricsInternalError, match=".*1020.*"):
            get_ossec_logs()

        with patch('securics.core.manager.exists', return_value=True):
...
def test_get_logs_summary(mock_exists, mock_active_logging_format):
    ...
    with patch('securics.core.manager.tail', return_value=logs):
        ...
        assert result['securics-modulesd:database'] == {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0,
                                                     'debug': 2}


@patch('securics.core.manager.exists', return_value=True)
@patch('securics.core.manager.SecuricsSocket')
def test_validate_ossec_conf(mock_securicssocket, mock_exists):
    with patch('socket.socket') as sock:
        # Mock sock response
        json_response = json.dumps({'error': 0, 'message': ""}).encode()
        mock_securicssocket.return_value.receive.return_value = json_response
        result = validate_ossec_conf()

        assert result == {'status': 'OK'}
        mock_exists.assert_called_with(os.path.join(common.SECURICS_PATH, 'queue', 'sockets', 'com'))
...
def test_validation_ko(mock_exists):
    ...
    with patch('socket.socket.bind'):
        ...
        with patch("securics.core.manager.exists", return_value=False):
            with pytest.raises(SecuricsInternalError, match='.* 1901 .*'):
                validate_ossec_conf()

        with patch('socket.socket.connect'):
            # Socket send raise socket.error
            with patch('securics.core.manager.SecuricsSocket.send', side_effect=socket.error):
                with pytest.raises(SecuricsInternalError, match='.* 1014 .*'):
                    validate_ossec_conf()

            with patch('socket.socket.send'):
                # Socket recv raise socket.error
                with patch('securics.core.manager.SecuricsSocket.receive', side_effect=socket.timeout):
                    with pytest.raises(SecuricsInternalError, match='.* 1014 .*'):
                        validate_ossec_conf()

                # _parse_execd_output raise KeyError
                with patch('securics.core.manager.SecuricsSocket'):
                    with patch('securics.core.manager.parse_execd_output', side_effect=KeyError):
                        with pytest.raises(SecuricsInternalError, match='.* 1904 .*'):
                            validate_ossec_conf()
...
def test_parse_execd_output(error_flag, error_msg):
    ...
    if not error_flag:
        ...
        with pytest.raises(SecuricsException, match=f'.* 1908 .*'):
            parse_execd_output(json_response)


@patch('securics.core.manager.configuration.api_conf', new={'experimental_features': True})
...
def test_get_update_information_template(last_check_date, update_check, installation_uid):
    ...
    assert template['current_version'] == f"v{securics.__version__}"
...
async def test_query_update_check_service_request(
    ...
    version = '4.8.0'
    with patch('framework.securics.core.manager.securics.__version__', version):
        ...
        client_session_get_mock.assert_called_with(
            ...
            headers={
                ...
                SECURICS_TAG_KEY: f'v{version}',
                USER_AGENT_KEY: f'Securics UpdateCheckService/v{version}'
            },

...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        from securics.core.agent import *
        from securics.core.exception import SecuricsException
        from api.util import remove_nones_to_dict
        from securics.core.common import reset_context_cache
...
def get_SecuricsDBQuery_params(wdb_class):
    """Get default parameters for the specified SecuricsDBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the SecuricsDBQuery class. Example: 'Agents' to get default parameters from `SecuricsDBQueryAgents` class.
    ...
    """
    with patch('securics.core.agent.SecuricsDBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'SecuricsDBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with Securics packages availables with their hash
...
@patch("securics.core.agent.SecuricsDBBackend")
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_SecuricsDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of SecuricsDBQueryAgents works properly.
    ...
    if value:
        SecuricsDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(SecuricsException, match=".* 2005 .*"):
            SecuricsDBQueryAgents()


@patch('socket.socket.connect')
def test_SecuricsDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of SecuricsDBQueryAgents returns expected query"""
    query_agent = SecuricsDBQueryAgents()
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of SecuricsDBQueryAgents returns expected result
    ...
    """
    query_agent = SecuricsDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of SecuricsDBQueryAgents returns expected query"""
    query_agent = SecuricsDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of SecuricsDBQueryAgents returns expected data"""
    query_agent = SecuricsDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of SecuricsDBQueryAgents returns expected query"""
    query_agent = SecuricsDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of SecuricsDBQueryAgents returns expected query
...
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_SecuricsDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of SecuricsDBQueryGroup works properly.
...
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_SecuricsDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of SecuricsDBQueryGroup works properly.
        ...
        """
    query_group = SecuricsDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of SecuricsDBQueryGroupByAgents works properly."""
    query_group = SecuricsDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of SecuricsDBQueryGroupByAgents works properly."""
    query_group = SecuricsDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'securics-master'},
                         {'count': 1, 'name': 'securics-agent1'}]
...
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_SecuricsDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if SecuricsDBQueryGroupByAgents works properly."""
    query_group = SecuricsDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_SecuricsDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of SecuricsDBQueryGroup works properly"""
    query_group = SecuricsDBQueryGroup()
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryMultigroups__init__(mock_socket_conn):
    """Tests if method __init__ of SecuricsDBQueryMultigroups works properly."""
    query_multigroups = SecuricsDBQueryMultigroups(group_id='test')
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryMultigroups_default_query(mock_socket_conn, group_id):
    """Tests if method _default_query of SecuricsDBQueryMultigroups works properly.
    ...
    """
    query_multigroups = SecuricsDBQueryMultigroups(group_id=group_id)
...
@patch('socket.socket.connect')
def test_SecuricsDBQueryMultigroups_default_count_query(mock_socket_conn):
    """Tests if method _default_count_query of SecuricsDBQueryMultigroups works properly."""
    query_multigroups = SecuricsDBQueryMultigroups(group_id='test')
...
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_SecuricsDBQueryMultigroups_get_total_items(mock_socket_conn, send_mock):
    """Tests if method _get_total_items of SecuricsDBQueryMultigroups works properly."""
    query_multigroups = SecuricsDBQueryMultigroups(group_id='test')
...
@patch('securics.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(SecuricsResourceNotFound, match='.* 1701 .*'):
...
def test_agent_get_key_ko(socket_mock, send_mock):
    ...
    with pytest.raises(SecuricsError, match='.* 1703 .*'):
        agent = Agent('000')
        agent.get_key()


@patch('securics.core.agent.SecuricsQueue.send_msg_to_agent')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(SecuricsQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(SecuricsQueue.HC_FORCE_RECONNECT, agent_id)


@patch('securics.core.agent.SecuricsQueue')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(SecuricsError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('securics.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('securics.core.agent.get_manager_status', return_value={'securics-authd': 'running'}):
        agent = Agent('000')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(SecuricsError, match='.* 1726 .*'):
        agent = Agent('000')
        agent.remove()


@patch('securics.core.agent.SecuricsSocketJSON')
def test_agent_remove_authd(mock_securics_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('000')
    agent._remove_authd(purge=True)
    mock_securics_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(0).zfill(3), "purge": True}})
    mock_securics_socket.return_value.receive.assert_called_once()
    mock_securics_socket.return_value.close.assert_called_once()
...
@patch('securics.core.agent.Agent._add_authd')
...
@patch('securics.core.agent.get_manager_status', return_value={'securics-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(SecuricsError, match='.* 1706 .*'):
...
@patch('securics.core.agent.SecuricsSocketJSON')
def test_agent_add_authd(mock_securics_socket, name, ip, id, key, force):
    ...
    mock_securics_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (SecuricsError(9008, cmd_error=True), ".* 1705 .*"),
    (SecuricsError(9007, cmd_error=True), ".* 1706 .*"),
    (SecuricsError(9012, cmd_error=True), ".* 1708 .*"),
    (SecuricsError(9000, cmd_error=True), ".* None")
])
@patch('securics.core.agent.SecuricsSocketJSON')
def test_agent_add_authd_ko(mock_securics_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(SecuricsError, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_securics_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(SecuricsError, match=expected_exception):
...
@patch('securics.core.agent.rmtree')
@patch('securics.core.agent.path.exists', return_value=True)
@patch('securics.core.common.SHARED_PATH', new=os.path.join(test_data_path, 'etc', 'shared'))
...
@patch('securics.core.agent.Agent.get_agent_groups', return_value=['default'])
@patch('securics.core.agent.Agent.set_agent_group_relationship')
...
def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with patch('securics.core.agent.Agent.get_agent_groups', side_effect=SecuricsError(2003)):
        with pytest.raises(SecuricsInternalError, match='.* 2007 .*'):
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(SecuricsError, match='.* 1722 .*'):
        Agent.group_exists('default**')


@patch('securics.core.agent.SecuricsDBConnection.send', return_value=('ok', '["payload"]'))
@patch('socket.socket.connect')
...
@patch('securics.core.agent.SecuricsDBConnection.send')
@patch('socket.socket.connect')
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(SecuricsInternalError, match='.* 2005 .*'):
...
@patch('securics.core.agent.Agent.set_agent_group_relationship')
@patch('securics.core.agent.Agent.group_exists', return_value=True)
@patch('securics.core.agent.Agent.get_basic_information')
def test_agent_unset_single_group_agent(agent_info_mock, group_exists_mock, set_agent_group_mock, agent_id, group_id,
    ...
    """
    with patch('securics.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
def test_agent_unset_single_group_agent_ko(socket_mock, agent_information_mock):
    ...
    with patch('securics.core.agent.Agent.group_exists', return_value=False):
        with pytest.raises(SecuricsResourceNotFound, match='.* 1710 .*'):
            Agent.unset_single_group_agent('002', 'test_group')

    # Agent does not belong to group
    with patch('securics.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(SecuricsError, match='.* 1734 .*'):
            Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('securics.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(SecuricsError, match='.* 1745 .*'):
            Agent.unset_single_group_agent('002', 'default', force=True)


@patch('securics.core.securics_socket.SecuricsSocket')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_securics_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_securics_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'Securics v4.0.0')
...
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_securics_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(SecuricsError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'Securics v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(SecuricsError, match=".* 1307 .*"):
        agent.get_config('com', None, 'Securics v4.0.0')
        agent.get_config(None, 'active-response', 'Securics v4.0.0')

    # Agent Securics version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(SecuricsInternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'Securics v3.6.0')


@patch('securics.core.securics_socket.SecuricsSocket')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_securics_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_securics_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_securics_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(SecuricsInternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['Securics v4.2.0', 'Securics v4.0.0', 'Securics v4.2.1', 'Securics v3.13.2']])
])
@patch('securics.core.agent.SecuricsQueue.send_msg_to_agent')
@patch('securics.core.agent.SecuricsQueue.__init__', return_value=None)
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    """
    with patch('securics.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            wq = SecuricsQueue(common.AR_SOCKET)
            send_restart_command(agent_id, agent_version['version'], wq)
            expected_msg = SecuricsQueue.RESTART_AGENTS_JSON if SecuricsVersion(
                agent_version['version']) >= SecuricsVersion(common.AR_LEGACY_VERSION) else SecuricsQueue.RESTART_AGENTS
...
def test_get_agents_info():
    ...
    with patch('securics.core.agent.open', mock_open(read_data=client_keys)) as m:
...
def test_get_groups():
    ...
    with patch('securics.core.common.SHARED_PATH', new=shared):
        try:
...
def test_expand_group(socket_mock, group, wdb_response, expected_agents):
    ...
    wdb_response: list
        Mock return values for the `SecuricsDBConnection.send` method.
    expected_agents : set
    ...
    with patch('securics.core.wdb.SecuricsDBConnection.send', side_effect=wdb_response):
...
@patch('securics.core.agent.core_upgrade_agents')

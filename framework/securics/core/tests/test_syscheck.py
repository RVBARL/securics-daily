...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        from securics.core import syscheck
        from securics.core.utils import get_date_from_timestamp


@pytest.mark.parametrize('agent', ['002', '080'])
@patch("securics.core.syscheck.SecuricsDBBackend")
@patch("securics.core.syscheck.SecuricsDBQuery.__init__")
def test_securics_db_query_syscheck__init__(mock_wdbquery, mock_backend, agent):
    """Test if SecuricsDBQuery and SecuricsDBBackend are called with the expected parameters.
    ...
    agent: str
        The agent to test.
    """
    syscheck.SecuricsDBQuerySyscheck(agent)
...
@patch("securics.core.syscheck.SecuricsDBBackend")
def test_securics_db_syscheck_format_data_into_dictionary(mock_backend, data, is_json):
    """Test if _format_data_into_dictionary() returns the expected element."""
    test = syscheck.SecuricsDBQuerySyscheck('002', offset=0, limit=1000, sort=None, search='test',

...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        ...
        from securics.core import common, stats
        from securics.core.exception import SecuricsError, SecuricsException, SecuricsInternalError
...
def test_totals_ko_():
    ...
    with patch('securics.core.stats.open', side_effect=IOError):
        with pytest.raises(SecuricsException, match=".* 1308 .*"):
            stats.totals_(date(1996, 8, 13))

    with patch("builtins.open", mock_open(read_data='15-571-3-2\n15--107--1483')):
        with pytest.raises(SecuricsInternalError, match=".* 1309 .*"):
            stats.totals_(date(1996, 8, 13))


@patch('securics.core.common.STATS_PATH', new=test_data_path)
def test_weekly_():
...
@patch('securics.core.common.STATS_PATH', new='')
...
@pytest.mark.parametrize('agent, daemon, expected_value', [
    (0, 'logcollector', os.path.join(common.SECURICS_PATH, "queue", "sockets", 'logcollector')),
...
@patch('securics.core.securics_socket.SecuricsSocketJSON.close')
@patch('securics.core.securics_socket.SecuricsSocketJSON.send')
@patch('securics.core.securics_socket.SecuricsSocketJSON.__init__', return_value=None)
def test_get_daemons_stats_socket(mock__init__, mock_send, mock_close, agents_list, expected_socket_response,
    ...
    with patch('securics.core.securics_socket.SecuricsSocketJSON.receive',
...
def test_get_daemons_stats_socket_ko(agents_list):
    ...
    socket = '/test_path/socket'
    with pytest.raises(SecuricsInternalError, match=f".* 1121 .*: {socket}"):
...
def test_get_daemons_stats_ko():
    ...
    with patch('securics.core.stats.open'):
        with pytest.raises(SecuricsInternalError, match=".* 1104 .*"):
            stats.get_daemons_stats_('filename')
...
def test_get_daemons_stats_from_socket(agent_id, daemon, response):
    ...
    with patch('securics.core.securics_socket.SecuricsSocket.__init__', return_value=None) as mock_socket:
        with patch('securics.core.securics_socket.SecuricsSocket.send', side_effect=None) as mock_send:
            with patch('securics.core.securics_socket.SecuricsSocket.receive', return_value=response.encode()):
                with patch('securics.core.securics_socket.SecuricsSocket.close', side_effect=None):
                    stats.get_daemons_stats_from_socket(agent_id, daemon)

        if agent_id == '000':
            mock_socket.assert_called_once_with(os.path.join(common.SECURICS_PATH, "queue", "sockets", "logcollector"))
            mock_send.assert_called_once_with(b'getstate')
        else:
            mock_socket.assert_called_once_with(os.path.join(common.SECURICS_PATH, "queue", "sockets", "remote"))
...
def test_get_daemons_stats_from_socket(agent_id, daemon, responses, expected, expected_socket_calls,
    ...
    with patch('securics.core.securics_socket.SecuricsSocket.__init__', return_value=None) as mock_socket:
        with patch('securics.core.securics_socket.SecuricsSocket.send', side_effect=None) as mock_send:
            with patch('securics.core.securics_socket.SecuricsSocket.receive', side_effect=responses):
                with patch('securics.core.securics_socket.SecuricsSocket.close', side_effect=None):
...
def test_get_daemons_stats_from_socket_ko():
    ...
    with pytest.raises(SecuricsError, match=r'\b1307\b'):
        stats.get_daemons_stats_from_socket(None, None)

    with pytest.raises(SecuricsError, match=r'\b1310\b'):
        stats.get_daemons_stats_from_socket('000', 'agent')

    with pytest.raises(SecuricsInternalError, match=r'\b1121\b'):
        stats.get_daemons_stats_from_socket('000', 'logcollector')

    with patch('securics.core.securics_socket.SecuricsSocket.__init__', return_value=None):
        with patch('securics.core.securics_socket.SecuricsSocket.close', side_effect=None):
            with patch('securics.core.securics_socket.SecuricsSocket.send', side_effect=None):
                with patch('securics.core.securics_socket.SecuricsSocket.receive', side_effect=ValueError):
                    with pytest.raises(SecuricsInternalError, match=r'\b1118\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')

                with patch('securics.core.securics_socket.SecuricsSocket.receive',
                           return_value=json.dumps({'error': 1}).encode()):
                    with pytest.raises(SecuricsError, match=r'\b1117\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')

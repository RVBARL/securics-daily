...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        from securics.core import sca as core_sca
...
@patch('securics.core.utils.SecuricsDBBackend.__init__', return_value=None)
@patch('securics.core.utils.SecuricsDBQuery.__init__')
def test_SecuricsDBQuerySCA__init__(mock_wdbq, mock_backend, mock_get_basic_info, distinct, agent_id, offset, limit, sort,
                                 search, query, count, get_data, select):
    """Test if method __init__ of SecuricsDBQuerySCA works properly.
    ...
    """
    wdbq_sca = core_sca.SecuricsDBQuerySCA(agent_id=agent_id, offset=offset, limit=limit, sort=sort, search=search,
    ...
    mock_wdbq.assert_called_once_with(ANY, offset=offset, limit=limit, table='sca_policy', sort=sort, search=search,
                                      select=select, fields=core_sca.SecuricsDBQuerySCA.DB_FIELDS,
...
def test_SecuricsDBQuerySCA__format_data_into_dictionary(agent_id, offset, limit, sort, search, select, query, count,
                                                      get_data):
    """Check if SecuricsDBQuerySCA's method _format_data_into_dictionary works properly.
...
@patch('securics.core.sca.SecuricsDBQuerySCA.__init__')
def test_SecuricsDBQuerySCACheck__init__(mock_wdbqsca, select, sca_checks_test_list, expected_default_query):
    """Test if method __init__ of SecuricsDBQuerySCACheck works properly.
    ...
    """
    core_sca.SecuricsDBQuerySCACheck(agent_id='000', select=select, sort={'fields': ['title'], 'order': 'asc'},
...
def test_SecuricsDBQuerySCACheck_parse_select_filter(mock_exists, mock_get_basic_info, mock_backend):
    """Test if method _parse_select_filter of SecuricsDBQuerySCACheck works properly."""
    wdbq_sca_check = core_sca.SecuricsDBQuerySCACheck(agent_id='000', sort={'value': 'test'},
...
@patch('securics.core.sca.SecuricsDBQuerySCA.__init__')
def test_SecuricsDBQuerySCACheckIDs__init__(mock_wdbqsca, query):
    """Test if method __init__ of SecuricsDBQuerySCACheckIDs works properly.

    Parameters
    ----------
    query : str
        Query used to initialize the SecuricsDBQuerySCACheckIDs object.
    """
    ...
    core_sca.SecuricsDBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
                                     search={'value': 'test'},
...
@patch('securics.core.utils.SecuricsDBBackend.__init__', return_value=None)
@patch('securics.core.agent.Agent.get_basic_information')
def test_SecuricsDBQuerySCACheckIDs_protected_pass_filter(mock_get_basic_info, mock_backend, field, value, expected):
    """Test SecuricsDBQuerySCACheckIDs._pass_filter function."""
    query = core_sca.SecuricsDBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
...
@patch('securics.core.sca.SecuricsDBQuerySCA.__init__')
def test_SecuricsDBQuerySCACheckRelational__init__(mock_wdbqsca, select, table, sca_checks_test_list):
    """Test if method __init__ of SecuricsDBQuerySCACheckRelational works properly.
    ...
    select : list or None
        Fields to select.
    table : str
        Table used to initialize the SecuricsDBQuerySCACheckRelational object.
    sca_checks_test_list : list
        List of SCA checks IDs.
    """
    query_sca_check_relational = core_sca.SecuricsDBQuerySCACheckRelational(agent_id='000', table=table,
...
@patch('securics.core.sca.SecuricsDBQuerySCA.__enter__')
@patch('securics.core.sca.SecuricsDBQuerySCA.__init__', return_value=None)
@patch('securics.core.sca.SecuricsDBQuery.__exit__')
def test_SecuricsDBQueryDistinctSCACheck__init__(mock_exit, mock_wdbqsca, mock_enter, select):
    """Test if method __init__ of SecuricsDBQueryDistinctSCACheck works properly."""
    ...
    core_sca.SecuricsDBQueryDistinctSCACheck(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
    ...
    mock_wdbqsca.assert_has_calls(
        [call(agent_id='000', offset=0, limit=None, sort={'fields': ['title'], 'order': 'asc'},
              ...
              default_query=core_sca.SecuricsDBQueryDistinctSCACheck.INNER_QUERY_PATTERN, search={'value': 'test'}),

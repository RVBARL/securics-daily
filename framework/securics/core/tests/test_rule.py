...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        ...
        from securics.core import rule
...
@pytest.mark.parametrize('rule_file, rule_path, rule_status, exception', [
    ...
    ('noexists.xml', 'tests/data/rules', 'enabled', SecuricsError(1201))
])
@patch("securics.core.common.SECURICS_PATH", new=parent_directory)
@patch("securics.core.common.RULES_PATH", new=data_path)
...
@patch("securics.core.rule.load_securics_xml", side_effect=OSError(13, 'Error', 'Permissions'))
def test_load_rules_from_file_permissions(mock_load):
    """Test set_groups rule core function."""
    with pytest.raises(SecuricsError, match='.* 1207 .*'):
        rule.load_rules_from_file('nopermissions.xml', 'tests/data/rules', 'disabled')


@patch("securics.core.rule.load_securics_xml", side_effect=OSError(8, 'Error', 'Unknown'))

...
class SecuricsDBQueryRootcheck(SecuricsDBQuery):
    """Rootcheck SecuricsDBQuery object."""
    fields = {
    ...
    def __init__(self, agent_id: str, offset: int, limit: int, sort: dict, search: dict, select: list, query: str,
        ...
        backend = SecuricsDBBackend(agent_id)
        SecuricsDBQuery.__init__(self, offset=offset, limit=limit, table='pm_event', sort=sort, search=search,
    ...
    def _filter_status(self, filter_status: dict):
        ...
        if filter_status['value'] == 'all':
            ...
            raise SecuricsException(1603, filter_status['value'])
...
def rootcheck_delete_agent(agent: str, wdb_conn: SecuricsDBConnection) -> None:

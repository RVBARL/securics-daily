...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        ...
        from securics.syscheck import run, clear, last_scan, files
        from securics.syscheck import AffectedItemsSecuricsResult
        from securics import SecuricsError, SecuricsInternalError
...
@patch('securics.core.common.CLIENT_KEYS', new=os.path.join(test_agent_data_path, 'client.keys'))
@patch('securics.syscheck.SecuricsDBQueryAgents.__exit__')
@patch('securics.syscheck.SecuricsDBQueryAgents.__init__', return_value=None)
@patch('securics.syscheck.SecuricsQueue._connect')
@patch('securics.syscheck.SecuricsQueue.send_msg_to_agent', side_effect=set_callable_list)
@patch('securics.syscheck.SecuricsQueue.close')
def test_syscheck_run(close_mock, send_mock, connect_mock, agent_init_mock, agent_exit_mock,
    ...
    """
    with patch('securics.syscheck.SecuricsDBQueryAgents.run', return_value=failed_items[0]):
...
@patch('securics.core.wdb.SecuricsDBConnection.__init__', return_value=None)
@patch('securics.core.wdb.SecuricsDBConnection.execute', return_value=None)
@patch('securics.core.wdb.SecuricsDBConnection.close')
def test_syscheck_clear(wdb_close_mock, wdb_execute_mock, wdb_init_mock, agent_list, expected_result, agent_info_list,
    ...
    """
    with patch('securics.syscheck.get_agents_info', return_value=set(agent_info_list)), \
            patch('securics.syscheck.SecuricsDBQueryAgents') as mock_wdbqa:
...
@patch('securics.core.wdb.SecuricsDBConnection.__init__', return_value=None)
@patch('securics.core.wdb.SecuricsDBConnection.execute', side_effect=SecuricsError(1000))
@patch('securics.core.wdb.SecuricsDBConnection.close')
...
@pytest.mark.parametrize('agent_id, securics_version', [
    (['001'], {'version': 'Securics v3.6.0'}),
    (['002'], {'version': 'Securics v3.8.3'}),
    (['005'], {'version': 'Securics v3.5.3'}),
    (['006'], {'version': 'Securics v3.9.4'}),
    (['004'], {}),
])
...
@patch("securics.syscheck.SecuricsDBConnection.execute", return_value=[{'end': '', 'start': ''}])
@patch('socket.socket.connect')
def test_syscheck_last_scan(socket_mock, wdb_conn_mock, db_mock, exists_mock, agent_id, securics_version):
    ...
    agent_id : list
        Agent ID.
    securics_version : dict
        Dict with the Securics version to be applied.
    """
    with patch('securics.syscheck.Agent.get_basic_information', return_value=securics_version):
...
@patch('securics.core.utils.path.exists', return_value=True)
@patch('socket.socket.connect')
@patch('securics.core.common.WDB_PATH', new=test_data_path)

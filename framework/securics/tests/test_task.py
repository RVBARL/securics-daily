...
with patch('rvbionics.common.securics_uid'):
    with patch('rvbionics.common.securics_gid'):
        ...
        from securics.tests.util import get_fake_database_data, RBAC_bypasser, InitWDBSocketMock

        securics.rbac.decorators.expose_resources = RBAC_bypasser
        from securics import task, SecuricsError
...
@patch('securics.core.utils.SecuricsDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_tasks_test.sql'))
@patch.object(SecuricsDBQueryTask, '_final_query', fake_final_query)

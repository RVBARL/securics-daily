...
with patch('securics.core.common.securics_uid'):
    with patch('securics.core.common.securics_gid'):
        ...
        from securics.agent import add_agent, assign_agents_to_group, create_group, delete_agents, delete_groups, \
...
@pytest.mark.parametrize('fields, expected_items', [
    ...
    (
            ['version'],
            [{'version': 'Securics v3.9.0', 'count': 1}, {'version': 'Securics v3.8.2', 'count': 2},
             {'version': 'Securics v3.6.2', 'count': 1}, {'version': 'N/A', 'count': 2}]
    ),
    (
...
@patch('securics.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsSecuricsResult), 'The returned object is not an "AffectedItemsSecuricsResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsSecuricsResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, SecuricsResult), 'The returned object is not an "SecuricsResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsSecuricsResult), 'The returned object is not an "SecuricsResult" instance.'
...
@patch('securics.core.agent.Agent.reconnect')
@patch('securics.agent.get_agents_info', return_value=short_agent_list)
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsSecuricsResult), 'The returned object is not an "AffectedItemsSecuricsResult".'
...
@patch('securics.agent.send_restart_command')
@patch('securics.agent.get_agents_info', return_value=set(short_agent_list))
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('securics.agent.get_agents')
@patch('securics.agent.get_groups')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~securics,status~active)', 'group=default;((name~securics,status~active))'),
    ('default', 'name~securics,status~active', 'group=default;(name~securics,status~active)')
])
@patch('securics.agent.get_agents')
@patch('securics.agent.get_groups')
...
@pytest.mark.parametrize('agent_list, filters, q, error_code, expected_items', [
    ...
    (full_agent_list[1:], {'status': 'all', 'older_than': '1s', 'version': 'Securics v3.6.2'}, None, 1731, ['002']),
    ...
    (['001', '002'], {'status': 'all', 'older_than': '1s'}, None, SecuricsError(1726), None),
])
@patch('securics.agent.Agent.remove')
@patch('securics.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_delete_agents(socket_mock, send_mock, mock_remove, agent_list, filters, q, error_code, expected_items):
    ...
    """
    if not isinstance(error_code, SecuricsException):
...
@patch('securics.core.agent.get_manager_status', return_value={'securics-authd': 'running'})
...
@patch('securics.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('securics.core.common.SHARED_PATH', new=test_shared_path)
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agent_groups_exceptions(socket_mock, send_mock, mock_get_groups, db_global, system_groups,
    ...
    with patch('securics.core.common.DATABASE_PATH_GLOBAL', new=db_global):
        try:
...
@patch('securics.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('securics.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('securics.core.common.SHARED_PATH', new=test_shared_path)
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, SecuricsResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, SecuricsError(1006)),
    (test_shared_path, ['default'], True, SecuricsError(1405), SecuricsError(1405)),
    (test_shared_path, ['default'], True, SecuricsException(1400), SecuricsInternalError(1727))
])
@patch('securics.agent.process_array')
@patch('securics.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('securics.core.common.SHARED_PATH', new=shared_path):
        ...
        except (SecuricsError, SecuricsInternalError) as e:
...
@patch('securics.core.common.SHARED_PATH', new=test_shared_path)
@patch('securics.core.common.securics_gid', return_value=getgrnam('root'))
@patch('securics.core.common.securics_uid', return_value=getpwnam('root'))
@patch('securics.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, SecuricsResult), 'The returned object is not an "SecuricsResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', SecuricsError, 1711),
    ('group-1', SecuricsError, 1711),
    ('invalid!', SecuricsError, 1722),
    ('delete-me', SecuricsInternalError, 1005),
    ('ar.conf', SecuricsError, 1713),
    ('agent-template.conf', SecuricsError, 1713)
])
@patch('securics.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the Securics Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('securics.agent.get_groups')
@patch('securics.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [SecuricsResourceNotFound(1710)]),
    (['default'], [SecuricsError(1712)]),
    (['none-1', 'none-2'], [SecuricsResourceNotFound(1710)]),
    (['default', 'none-1'], [SecuricsError(1712), SecuricsResourceNotFound(1710)]),
])
@patch('securics.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of SecuricsError
        List of expected SecuricsError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('securics.agent.Agent.add_group_to_agent')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], SecuricsResourceNotFound(1710), True),
    (['group-1'], ['100'], SecuricsResourceNotFound(1701), False),
    (['default'], ['000'], SecuricsError(1703), False)
])
@patch('securics.agent.Agent.group_exists')
@patch('securics.agent.Agent.add_group_to_agent')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    catch_exception : bool
        ...
        `AffectedItemsSecuricsResult` containing the exceptions in its 'failed_items'.
    """
    ...
    except SecuricsException as ex:
...
@patch('securics.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('securics.core.agent.Agent.unset_single_group_agent')
@patch('securics.agent.get_groups')
@patch('securics.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', SecuricsResourceNotFound(1701)),
    ('any-group', '000', SecuricsError(1703)),
    ('group-1', '005', SecuricsResourceNotFound(1710)),
])
@patch('securics.agent.get_agents_info', return_value=short_agent_list)
@patch('securics.agent.get_groups', side_effect={'default'})
def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : SecuricsError
        The SecuricsError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (SecuricsError, SecuricsResourceNotFound) as error:
...
@patch('securics.core.agent.Agent.unset_single_group_agent')
@patch('securics.agent.get_agents_info', return_value=short_agent_list)
@patch('securics.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], SecuricsResourceNotFound(1701), True),
    (['any-group'], ['000'], SecuricsError(1703), True),
    (['any-group'], ['005'], SecuricsResourceNotFound(1710), False),
])
@patch('securics.core.agent.Agent.unset_single_group_agent')
@patch('securics.agent.get_agents_info', return_value=short_agent_list)
@patch('securics.agent.get_groups', return_value={'group-1'})
def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            f'An "SecuricsError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsSecuricsResult), 'The returned object is not an "AffectedItemsSecuricsResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            f'No exception should be raised at this point. An AffectedItemsSecuricsResult object with at least one ' \
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['non-group'], ['000'], SecuricsResourceNotFound(1710), True),
    (['group-1'], ['000'], SecuricsError(1703), False),
    (['group-1'], ['100'], SecuricsResourceNotFound(1701), False),
])
@patch('securics.agent.get_agents_info', return_value=short_agent_list)
@patch('securics.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating SecuricsInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
@patch('securics.agent.get_agents_info', return_value=set(full_agent_list))
@patch('securics.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_upgrade_agents(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    """
    with patch('securics.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        core_upgrade_agents_mock.return_value = result_from_socket

        if raise_error:
            # Upgrade expecting a Securics Exception
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    with pytest.raises(SecuricsError, match=f".* {error} .*"):
                    ...
                    with pytest.raises(SecuricsInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('securics.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a Securics Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(SecuricsInternalError, match=f".* {error} .*"):
...
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, securics_socket_mock, agent_list, component, configuration):
    ...
    """
    securics_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
def test_agent_get_agent_config_exceptions(socket_mock, send_mock, agent_list):
    ...
    try:
        ...
        assert error == SecuricsError(1740)
...
@patch('securics.core.common.SHARED_PATH', new=test_shared_path)
@patch('securics.core.common.MULTI_GROUPS_PATH', new=test_multigroup_path)
@patch('securics.agent.get_agents_info', return_value=full_agent_list)
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('agent_list, expected_error', [
    (['000'], SecuricsError(1703)),
    (['100'], SecuricsResourceNotFound(1701))
])
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_agents_sync_group_exceptions(socket_mock, send_mock, agent_list, expected_error):
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : SecuricsError
        Expected SecuricsError to be returned (not raised) by the function.
    """
    result = get_agents_sync_group(agent_list=agent_list)
    assert isinstance(result, AffectedItemsSecuricsResult), 'The returned object is not an "AffectedItemsSecuricsResult".'
    # Check failed items
...
@patch('securics.core.common.SHARED_PATH', new=test_shared_path)
@patch('securics.core.configuration.upload_group_configuration')
...
@patch('securics.core.common.SHARED_PATH', new=test_shared_path)
@patch('securics.agent.get_distinct_agents')
@patch('securics.agent.get_agent_groups')
@patch('securics.agent.get_agents_summary_status')
@patch('securics.agent.get_agents')
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('securics.agent.get_agents_info', return_value=['test', 'test2'])
@patch('securics.core.wdb.SecuricsDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('securics.agent.get_agents_info', return_value=set(agent_ids_format(range(100000)))):
...
@patch('securics.core.agent.Agent.get_agent_groups')
@patch('securics.core.agent.Agent.set_agent_group_file')
@patch('securics.core.agent.Agent')
...
@patch('securics.core.agent.Agent.group_exists', return_value=False)
...
def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected SecuricsException code error.
    """
    with pytest.raises(SecuricsException, match=f".* {expected_exc} .*"):

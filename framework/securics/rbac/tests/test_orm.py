...
from securics.rbac.tests.utils import init_db
from securics.rbac.orm import SECURICS_USER_ID, SECURICS_WUI_USER_ID, MAX_ID_RESERVED, User
        ...
        with patch('sqlalchemy.create_engine', return_value=create_engine("sqlite://")):
            with patch('shutil.chown'), patch('os.chmod'):
                with patch('api.constants.SECURITY_PATH', new=test_data_path):
                    import securics.rbac.orm as rbac
                    # Clear mappers
...
def fresh_in_memory_db():
    ...
    with patch('securics.core.common.securics_uid'), patch('securics.core.common.securics_gid'):
        import securics.rbac.orm as orm
        reload(orm)
...
def test_database_init(db_setup):
    ...
    with db_setup.RolesManager() as rm:
        assert rm.get_role('securics') != db_setup.SecurityError.ROLE_NOT_EXIST
...
def test_delete_all_expired_rules(db_setup):
    """Check that rules are correctly deleted"""
    with patch('securics.rbac.orm.time', return_value=0):
        add_token(db_setup)
    with db_setup.TokenManager() as tm:
...
@patch("securics.rbac.orm.create_engine")
@patch("securics.rbac.orm.sessionmaker")
...
@patch("securics.rbac.orm._Base.metadata.create_all")
...
def test_databasemanager_get_table(fresh_in_memory_db):
    ...
    with patch("securics.rbac.orm._new_columns", new={"new_column"}):
...
@patch("securics.rbac.orm.safe_move")
@patch("securics.rbac.orm.os.remove")
@patch("securics.rbac.orm.chown")
@patch("securics.rbac.orm.os.chmod")
def test_check_database_integrity(chmod_mock, chown_mock, remove_mock, safe_move_mock, fresh_in_memory_db):
    ...
    db_mock = MagicMock()
    with patch("securics.rbac.orm.db_manager", new=db_mock):
        with patch("securics.rbac.orm.os.path.exists", return_value=True):
            with patch("securics.rbac.orm.CURRENT_ORM_VERSION", new=99999):
                ...
                db_mock.assert_has_calls([
                    ...
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
                                      from_id=fresh_in_memory_db.SECURICS_USER_ID,
                                      to_id=fresh_in_memory_db.SECURICS_WUI_USER_ID),
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
        ...
        safe_move_mock.assert_called_once_with(fresh_in_memory_db.DB_FILE_TMP, fresh_in_memory_db.DB_FILE,
                                               ownership=(fresh_in_memory_db.securics_uid(),
                                                          fresh_in_memory_db.securics_gid()),
                                               permissions=0o640)
...
@patch("securics.rbac.orm.DatabaseManager.close_sessions")
@patch("securics.rbac.orm.os.remove")
def test_check_database_integrity_exceptions(remove_mock, close_sessions_mock, exception, fresh_in_memory_db):
    ...
    with patch("securics.rbac.orm.os.path.exists", side_effect=mocked_exists) as mock_exists:
        with patch("securics.rbac.orm.DatabaseManager.connect", side_effect=exception) as db_manager_mock:
...
@pytest.mark.parametrize('from_id, to_id, users', [
    (SECURICS_USER_ID, SECURICS_WUI_USER_ID, [
        User('securics', 'test', user_id=SECURICS_USER_ID),
        User('securics-wui', 'test2', user_id=SECURICS_WUI_USER_ID)
    ]),
...
def test_migrate_data(db_setup, from_id, to_id, users):
    ...
    if to_id == SECURICS_WUI_USER_ID:
        with patch("securics.rbac.orm.db_manager.get_data", return_value=users):
            with patch("securics.rbac.orm.AuthenticationManager.update_user") as mock_update_user:
        ...
        empty_list = []
        user_exists = False
        with patch("securics.rbac.orm.AuthenticationManager.add_user",
                   side_effect=[not user_exists, user_exists, not user_exists]) as mock_add_user:
            with patch("securics.rbac.orm.db_manager.get_data",

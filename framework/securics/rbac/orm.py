...
from securics.core.common import securics_uid, securics_gid, DEFAULT_RBAC_RESOURCES
from securics.core.utils import get_utc_now, safe_move
from securics.rbac.utils import clear_cache

logger = logging.getLogger("securics-api")

# Max reserved ID value
SECURICS_USER_ID = 1
SECURICS_WUI_USER_ID = 2
MAX_ID_RESERVED = 99
    ...
    def migrate_data(self, source: str, target: str, from_id: int = None, to_id: int = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                if user.id in (SECURICS_USER_ID, SECURICS_WUI_USER_ID):
        ...
        if from_id == SECURICS_USER_ID and to_id == SECURICS_WUI_USER_ID:
            return
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set Securics ownership and permissions.
        ...
        """
        chown(database, securics_uid(), securics_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=SECURICS_USER_ID,
                                        to_id=SECURICS_WUI_USER_ID)
                ...
                safe_move(DB_FILE_TMP, DB_FILE,
                          ownership=(securics_uid(), securics_gid()),
                          permissions=0o640)
        ...
        logger.error("Error retrieving the current Securics RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e

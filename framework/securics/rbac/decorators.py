...
from securics.core.agent import get_agents_info, get_groups, expand_group
from securics.core.common import rbac, broadcast, cluster_nodes
from securics.core.exception import SecuricsPermissionError
from securics.core.results import AffectedItemsSecuricsResult
from securics.core.utils import expand_rules, expand_lists, expand_decoders
from securics.rbac.orm import RolesManager, PoliciesManager, AuthenticationManager, RulesManager
...
def list_handler(result: AffectedItemsSecuricsResult, original: dict = None, allowed: dict = None, target: dict = None,
                 add_denied: bool = False, **post_proc_kwargs: dict) -> AffectedItemsSecuricsResult:
        ...
        for res_id, target_param in target.items():
            ...
            for denied_item in denied:
                result.add_failed_item(id_=denied_item,
                                       error=SecuricsPermissionError(4000, extra_message=f'Resource type: {res_id}',
                                                                  ids=denied))
    if not add_denied or post_proc_kwargs.get('force'):
        ...
        if 'default_result_kwargs' in post_proc_kwargs and result is None:
            return AffectedItemsSecuricsResult(**post_proc_kwargs['default_result_kwargs'])
...
def expose_resources(actions: list = None, resources: list = None, post_proc_func: callable = list_handler,
    ...
    """Decorator to apply user permissions on a Securics framework function based on exposed action:resource pairs.
            ...
            for res_id, target_param in target_params.items():
                try:
                    ...
                    if add_denied:
                        ...
                        if res_id in integer_resources:
                            denied = {int(i) if i.isdigit() else i for i in denied}
                        raise SecuricsPermissionError(4000,

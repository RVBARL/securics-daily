...
from securics.core.exception import SecuricsInternalError, SecuricsError, SecuricsResourceNotFound
from securics.core.results import AffectedItemsSecuricsResult
from securics.core.syscheck import SecuricsDBQuerySyscheck, syscheck_delete_agent
...
def run(agent_list: Union[str, None] = None) -> AffectedItemsSecuricsResult:
    ...
    """
    result = AffectedItemsSecuricsResult(all_msg='Syscheck scan was restarted on returned agents',
    ...
    with SecuricsDBQueryAgents(limit=None, select=["id", "status"], query='status!=active', **rbac_filters) as db_query:
...
def clear(agent_list: list = None) -> AffectedItemsSecuricsResult:
    ...
    AffectedItemsSecuricsResult
        Confirmation/Error message.
    """
    result = AffectedItemsSecuricsResult(all_msg='Syscheck database was cleared on returned agents',
    ...
    list(map(lambda ag: result.add_failed_item(id_=ag, error=SecuricsResourceNotFound(1701)), not_found_agents))

    wdb_conn = None
    rbac_filters = get_rbac_filters(system_resources=system_agents, permitted_resources=agent_list)
    db_query = SecuricsDBQueryAgents(select=["id", "version"], **rbac_filters)
    data = db_query.run()
    ...
    for item in data['items']:
        ...
        if agent_version is not None:
            if SecuricsVersion(agent_version) < SecuricsVersion('v3.12.0'):
                try:
                    if wdb_conn is None:
                ...
                result.add_failed_item(id_=agent_id,
                                       error=SecuricsError(1760, extra_message="Agent version should be < v3.12.0."))
        else:
            result.add_failed_item(id_=agent_id, error=SecuricsError(1015))
...
def last_scan(agent_list: list) -> AffectedItemsSecuricsResult:
    ...
    result = AffectedItemsSecuricsResult(all_msg='Last syscheck scan of the agent was returned',
    ...
    with SecuricsDBQuerySyscheck(agent_id=agent_list[0], query='module=fim', offset=0, sort=None,
...
def files(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
          ...
          summary: bool = False, distinct: bool = False) -> AffectedItemsSecuricsResult:
    ...
    result = AffectedItemsSecuricsResult(all_msg='FIM findings of the agent were returned',
    ...
    with SecuricsDBQuerySyscheck(agent_id=agent_list[0], offset=offset, limit=limit, sort=sort, search=search,

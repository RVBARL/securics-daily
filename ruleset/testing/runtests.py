...
def getSecuricsInfo(securics_home):
    securics_control = os.path.join(securics_home, "bin", "securics-control")
    securics_env_vars = {}
    try:
        proc = subprocess.Popen([securics_control, "info"], stdout=subprocess.PIPE)
        (stdout, stderr) = proc.communicate()
    except Exception as e:
        print("Seems like there is no Securics installation.")
        return None
...
def provisionDR():
    ...
    for file in os.listdir(rules_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?rules.xml$', file):
            shutil.copy2(file_fullpath, args.securics_home + "/etc/rules")
    ...
    for file in os.listdir(decoders_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?decoders.xml$', file):
            shutil.copy2(file_fullpath, args.securics_home + "/etc/decoders")



def cleanDR():
    rules_dir = args.securics_home + "/etc/rules"
    decoders_dir = args.securics_home + "/etc/decoders"
...
class OssecTester(object):
    ...
    def buildCmd(self, rule, alert, decoder):
        cmd = ['%s/securics-logtest' % (self._ossec_path), ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='This script tests Securics rules.')
    parser.add_argument('--path', '-p', default='/var/ossec', dest='securics_home',
                        help='Use -p or --path to specify Securics installation path')
    ...
    securics_info = getSecuricsInfo(args.securics_home)
    if securics_info is None:
        sys.exit(1)
    ...
    provisionDR()
    OT = OssecTester(args.securics_home)

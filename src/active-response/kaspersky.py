#!/usr/bin/env python
 ...
 ###
 # Integration of Securics agent with Kaspersky endpoint security for Linux
...
# Sets the file paths
##################################################################################################################
securics_path = os.path.abspath(os.path.join(__file__, "../../.."))
securics_queue = '{0}/queue/sockets/queue'.format(securics_path)
now = time.strftime("%a %b %d %H:%M:%S %Z %Y")
ar_log_file = '{0}/logs/active-responses.log'.format(securics_path)
...
def send_msg(securics_queue, msg):
	...
	try:
		s.connect(securics_queue)
	except:
		print('Error: Securics must be running.')
		sys.exit(1)
...
def main():
    ar_log()
    set_logger('securics-kaspersky', foreground=args.verbose)
    run_kaspersky()

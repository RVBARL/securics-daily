...
# Install functions for Securics
# Securics.com (https://github.com/securics)
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/securics.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/securics.pp /tmp && semodule -i /tmp/securics.pp
                rm -f /tmp/securics.pp
                semodule -e securics
            fi
        fi
    fi
...
SecuricsUpgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown securics:securics $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for Securics DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/securics
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated Securics tools
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:securics
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown securics:securics
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:securics {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown securics:securics {} \;
        fi
    fi

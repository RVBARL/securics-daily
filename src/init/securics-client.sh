...
AUTHOR="Securics Inc."
DAEMONS="securics-modulesd securics-logcollector securics-syscheckd securics-agentd securics-execd"
...
check_folders()
{
    ...
    if [ ! -d $ALERTS_FOLDER ]
    then
        if rm -rf $ALERTS_FOLDER && mkdir -p $ALERTS_FOLDER && chown securics:securics $ALERTS_FOLDER && chmod 770 $ALERTS_FOLDER
        then

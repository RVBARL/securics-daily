...
AUTHOR="Securics Inc."
DAEMONS="securics-modulesd securics-monitord securics-logcollector securics-syscheckd securics-analysisd securics-maild securics-execd securics-db securics-agentlessd securics-integratord securics-dbd securics-csyslogd"
...
start_service()
{
    ...
    TEST=$(${DIR}/bin/securics-logtest-legacy -t  2>&1)
    echo $TEST

    if [ ! -z "$TEST" ]; then
        echo "securics-analysisd: Configuration error. Exiting."
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/securics-control start
    fi
...
stop_service()
{
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/securics-control stop
    fi

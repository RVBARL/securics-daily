...
# securics-control        This shell script takes care of starting
...
AUTHOR="Securics Inc."
USE_JSON=false
DAEMONS="securics-clusterd securics-modulesd securics-monitord securics-logcollector securics-remoted securics-syscheckd securics-analysisd securics-maild securics-execd securics-db securics-authd securics-agentlessd securics-integratord securics-dbd securics-csyslogd securics-apid"
OP_DAEMONS="securics-clusterd securics-maild securics-agentlessd securics-integratord securics-dbd securics-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting Securics $VERSION..."
    fi

    TEST=$(${DIR}/bin/securics-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/securics-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If securics-maild is disabled, don't try to start it.
        if [ X"$i" = "Xsecurics-maild" ]; then
             grep "<email_notification>no<" ${DIR}/etc/ossec.conf >/dev/null 2>&1
             if [ $? = 0 ]; then
                 continue
             fi
        fi
        ## If securics-clusterd is disabled, don't try to start it.
        if [ X"$i" = "Xsecurics-clusterd" ]; then
        ...
        ## If securics-authd is disabled, don't try to start it.
        if [ X"$i" = "Xsecurics-authd" ]; then
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by Securics, removing..."
                fi
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"SECURICS_VERSION":"'${VERSION}'"},'
            echo -n '{"SECURICS_REVISION":"'${REVISION}'"},'
            echo -n '{"SECURICS_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "SECURICS_VERSION=\"${VERSION}\""
            echo "SECURICS_REVISION=\"${REVISION}\""
            echo "SECURICS_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/securics-execd//')
    restart_service
    ;;
status)
    lock

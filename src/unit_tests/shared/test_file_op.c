...
{
    char *argv0 = "/usr/share/securics/bin/test";
    ...
    expect_string(__wrap_stat, __file, "/usr/share/securics");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/usr/share/securics");
    free(val);
}
...
void test_w_homedir_env_var(void **state)
{
    ...
    expect_string(__wrap_getenv, name, SECURICS_HOME_ENV);
    will_return(__wrap_getenv, "/home/securics");

    expect_string(__wrap_stat, __file, "/home/securics");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/home/securics");
    free(val);
}
...
void test_w_homedir_stat_fail(void **state)
{
    ...
    expect_string(__wrap__merror_exit, formatted_msg, "(1108): Unable to find Securics install directory. Export it to SECURICS_HOME environment variable.");

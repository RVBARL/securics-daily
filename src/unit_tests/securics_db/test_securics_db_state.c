...
#include "../securics_db/wdb_state.h"
...
static int test_setup(void ** state) {
    ...
    wdb_state.queries_breakdown.securicsdb_queries = 212;
    wdb_state.queries_breakdown.securicsdb_breakdown.remove_queries = 212;
    wdb_state.queries_breakdown.securicsdb_breakdown.remove_time.tv_sec = 0;
    wdb_state.queries_breakdown.securicsdb_breakdown.remove_time.tv_usec = 132156;
...
void test_securicsdb_create_state_json(void ** state) {
    ...
    *state = (void *)state_json;
    ...
    assert_non_null(cJSON_GetObjectItem(received_breakdown, "securicsdb"));
    assert_int_equal(cJSON_GetObjectItem(received_breakdown, "securicsdb")->valueint, 212);

    cJSON* securicsdb_queries_breakdown = cJSON_GetObjectItem(received_breakdown, "securicsdb_breakdown");

    cJSON* securicsdb_queries_db = cJSON_GetObjectItem(securicsdb_queries_breakdown, "db");
    assert_non_null(cJSON_GetObjectItem(securicsdb_queries_db, "remove"));
    assert_int_equal(cJSON_GetObjectItem(securicsdb_queries_db, "remove")->valueint, 212);
    ...
    assert_non_null(cJSON_GetObjectItem(execution_breakdown, "securicsdb"));
    assert_int_equal(cJSON_GetObjectItem(execution_breakdown, "securicsdb")->valueint, 132);

    cJSON* securicsdb_time_breakdown = cJSON_GetObjectItem(execution_breakdown, "securicsdb_breakdown");

    cJSON* securicsdb_time_db = cJSON_GetObjectItem(securicsdb_time_breakdown, "db");
    assert_non_null(cJSON_GetObjectItem(securicsdb_time_db, "remove"));
    assert_int_equal(cJSON_GetObjectItem(securicsdb_time_db, "remove")->valueint, 132);
}

int main(void) {
    const struct CMUnitTest tests[] = {
        // Test wdb_create_state_json
        cmocka_unit_test_setup_teardown(test_securicsdb_create_state_json, test_setup, test_teardown),
    };

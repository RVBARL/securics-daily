/* securicsdb state management functions
 * May 27, 2022
 *
...
void w_inc_securicsdb() {
    w_mutex_lock(&db_state_t_mutex);
    wdb_state.queries_breakdown.securicsdb_queries++;
    w_mutex_unlock(&db_state_t_mutex);
}

void w_inc_securicsdb_remove() {
    w_mutex_lock(&db_state_t_mutex);
    wdb_state.queries_breakdown.securicsdb_breakdown.remove_queries++;
    w_mutex_unlock(&db_state_t_mutex);
}

void w_inc_securicsdb_remove_time(struct timeval time) {
    w_mutex_lock(&db_state_t_mutex);
    timeradd(&wdb_state.queries_breakdown.securicsdb_breakdown.remove_time, &time, &wdb_state.queries_breakdown.securicsdb_breakdown.remove_time);
    w_mutex_unlock(&db_state_t_mutex);
}
...
cJSON* wdb_create_state_json() {
    ...
    cJSON_AddNumberToObject(_received_breakdown, "securicsdb", wdb_state_cpy.queries_breakdown.securicsdb_queries);

    cJSON *_securicsdb_breakdown = cJSON_CreateObject();
    cJSON_AddItemToObject(_received_breakdown, "securicsdb_breakdown", _securicsdb_breakdown);

    cJSON *_securicsdb_db = cJSON_CreateObject();
    cJSON_AddItemToObject(_securicsdb_breakdown, "db", _securicsdb_db);

    cJSON_AddNumberToObject(_securicsdb_db, "remove", wdb_state_cpy.queries_breakdown.securicsdb_breakdown.remove_queries);
    ...
    cJSON_AddNumberToObject(_execution_breakdown, "securicsdb", timeval_to_milis(wdb_state_cpy.queries_breakdown.securicsdb_breakdown.remove_time));

    cJSON *_securicsdb_breakdown_t = cJSON_CreateObject();
    cJSON_AddItemToObject(_execution_breakdown, "securicsdb_breakdown", _securicsdb_breakdown_t);

    cJSON *_securicsdb_db_t = cJSON_CreateObject();
    cJSON_AddItemToObject(_securicsdb_breakdown_t, "db", _securicsdb_db_t);

    cJSON_AddNumberToObject(_securicsdb_db_t, "remove", timeval_to_milis(wdb_state_cpy.queries_breakdown.securicsdb_breakdown.remove_time));
...
STATIC uint64_t get_time_total(wdb_state_t *state){
    return get_task_time(state) + get_global_time(state) + get_agent_time(state) + timeval_to_milis(state->queries_breakdown.securicsdb_breakdown.remove_time) + timeval_to_milis(state->queries_breakdown.mitre_breakdown.sql_time);
}

...
size_t wcom_dispatch(char *command, char ** output) {
    ...
    } else if (strcmp(rcv_comm, "restart") == 0 || strcmp(rcv_comm, "restart-securics") == 0) {
...
size_t wcom_restart(char ** output) {
    ...
    if (lock <= 0) {
        ...
        if (access("active-response/bin/restart.sh", F_OK) == 0) {
            ...
            exec_cmd[0] = "bin/securics-control";
            exec_cmd[1] = "restart";
        }
        ...
        snprintf(command, sizeof(command), "%s/%s", AR_BINDIR, "restart-securics.exe");
        char *cmd[2] = { command, NULL };
        char *cmd_parameters = "{\"version\":1,\"origin\":{\"name\":\"\",\"module\":\"securics-execd\"},\"command\":\"add\",\"parameters\":{\"extra_args\":[],\"alert\":{},\"program\":\"restart-securics.exe\"}}";
...
size_t wcom_check_manager_config(char **output) {
    static const char *daemons[] = {"bin/securics-authd", "bin/securics-remoted",
                                    "bin/securics-execd", "bin/securics-analysisd", "bin/securics-logcollector",
                                    "bin/securics-integratord", "bin/securics-syscheckd", "bin/securics-maild",
                                    "bin/securics-modulesd", "bin/securics-clusterd", "bin/securics-agentlessd",
                                    "bin/securics-integratord", "bin/securics-dbd", "bin/securics-csyslogd", NULL
                                    };

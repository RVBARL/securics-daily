...
/** Securics to prelude severity mapping. **/
static const char *(securics2prelude_sev[]) = {"info", "info", "info", "info",
                               "low", "low", "low", "low",
...
add_idmef_object(idmef_message_t *msg, const char *object, const char *value)
{
    ...
    if (ret < 0) {
        merror("Securics2Prelude: IDMEF: Cannot add object '%s': %s.",
...
err:
    merror("Securics2Prelude: %s: IDMEF error: %s.",
...
void OS_PreludeLog(const Eventinfo *lf)
{
    ...
    if ( ret < 0 ) {
        merror("Securics2Prelude: Cannot create IDMEF message");
        return;
    }
    ...
    add_idmef_object(idmef, "alert.assessment.impact.severity",
                     ...
                     securics2prelude_sev[lf->generated_rule->level]);
    ...
    /* Begin Classification Infomations */
    {
        ...
        /* Check Securics rules for reference */
        if (lf->generated_rule->sigid) {
            ...
            add_idmef_object(idmef, "alert.classification.reference(-1).meaning", "Securics Ruleset");
         ...
         * that points back to Securics ruleset for more infomation.
         */

...
int DecodeSyscollector(Eventinfo *lf,int *socket)
{
    ...
    if (strcmp(msg_type, "port") == 0 || strcmp(msg_type, "port_end") == 0) {
        if (decode_port(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send ports information to Securics DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_package(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send packages information to Securics DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_hotfix(lf, logJSON, socket) < 0) {
            mdebug1("Unable to send hotfixes information to Securics DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_hardware(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send hardware information to Securics DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_osinfo(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send osinfo message to Securics DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_netinfo(lf, logJSON, socket) < 0) {
            merror("Unable to send netinfo message to Securics DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_process(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send processes information to Securics DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
                    ...
                    if (NULL != data) {
                        ...
                        if (ret_val == 0) {
                            if (strncmp(response, "err", 3) == 0) {
                                /* If some error come to this point, it means that the error comes from securics-db. */
                            ...
                            /* If the return value is not 0, it means that the query to securics-db failed. */

...
static LPTSTR g_lpszServiceName        = "SecuricsSvc";
static LPTSTR g_lpszServiceDisplayName = "Securics";
static LPTSTR g_lpszServiceDescription = "Securics Windows Agent";
...
int os_stop_service()
{
    ...
    * newer instances of securics-agent.
    */
    ...
    if (ossecServiceStatusHandle) {
        switch (dwOpcode) {
            case SERVICE_CONTROL_STOP:
                ...
                // Kill children processes spawned by modules, only in securics-agent
                wm_kill_children();
                stop_wmodules();

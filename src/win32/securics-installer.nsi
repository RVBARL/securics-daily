...
!define NAME "Securics"
!define SERVICE "SecuricsSvc"

; output file
!ifndef OutFile
    !define OutFile "securics-agent-${VERSION}.exe"
!endif

Var is_upgrade

Name "${NAME} Windows Agent v${VERSION}"
BrandingText "Copyright (C) 2023-2024, RV Bionics Group SpA."
OutFile "${OutFile}"

VIProductVersion "4.9.0.0"
VIAddVersionKey ProductName "${NAME}"
VIAddVersionKey CompanyName "Securics Inc."
VIAddVersionKey LegalCopyright "2023 - Securics Inc."
VIAddVersionKey FileDescription "Securics Agent installer"
VIAddVersionKey FileVersion "${VERSION}"
VIAddVersionKey ProductVersion "${VERSION}"
VIAddVersionKey InternalName "Securics Agent"
...
Section "Securics Agent (required)" MainSec
    ...
    ; install files
    File securics-agent.exe
    File securics-agent-eventchannel.exe
    ...
    File /oname=active-response\bin\restart-securics.exe restart-securics.exe
    ...
    File /oname=libsecuricsext.dll ..\libsecuricsext.dll
    File /oname=libsecuricsshared.dll ..\libsecuricsshared.dll
    ...
    File /oname=syscollector.dll ..\securics_modules\syscollector\build\bin\syscollector.dll
    File /oname=libfimdb.dll ..\syscheckd/build/bin/libfimdb.dll
    File /oname=queue\syscollector\norm_config.json ..\securics_modules\syscollector\norm_config.json
    File VERSION
    File REVISION
    ...
    ; use appropriate version of "securics-agent.exe"
    ${If} ${AtLeastWinVista}
        Delete "$INSTDIR\securics-agent.exe"
        Rename "$INSTDIR\securics-agent-eventchannel.exe" "$INSTDIR\securics-agent.exe"
    ${Else}
        Delete "$INSTDIR\securics-agent-eventchannel.exe"
    ${Endif}
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "Publisher" "Securics, Inc."
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "DisplayIcon" '"$INSTDIR\favicon.ico"'
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "HelpLink" "https://rvbionics.com"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "URLInfoAbout" "https://rvbionics.com"
    ...
    ServiceInstall:
        nsExec::ExecToLog '"$INSTDIR\securics-agent.exe" install-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
...
Section "Uninstall"
    ...
    ServiceUninstall:
        nsExec::ExecToLog '"$INSTDIR\securics-agent.exe" uninstall-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\

...
' Copyright (C) 2023-2024, RV Bionics Group SpA. <support@rvbionics.com>
'
...
public function config()
    ...
    SECURICS_MANAGER = Replace(args(2), Chr(34), "")
    SECURICS_MANAGER_PORT = Replace(args(3), Chr(34), "")
    SECURICS_PROTOCOL = Replace(args(4), Chr(34), "")
    NOTIFY_TIME = Replace(args(5), Chr(34), "")
    SECURICS_REGISTRATION_SERVER = Replace(args(6), Chr(34), "")
    SECURICS_REGISTRATION_PORT = Replace(args(7), Chr(34), "")
    SECURICS_REGISTRATION_PASSWORD = Replace(args(8), Chr(34), "")
    SECURICS_KEEP_ALIVE_INTERVAL = Replace(args(9), Chr(34), "")
    SECURICS_TIME_RECONNECT = Replace(args(10), Chr(34), "")
    SECURICS_REGISTRATION_CA = Replace(args(11), Chr(34), "")
    SECURICS_REGISTRATION_CERTIFICATE = Replace(args(12), Chr(34), "")
    SECURICS_REGISTRATION_KEY = Replace(args(13), Chr(34), "")
    SECURICS_AGENT_NAME = Replace(args(14), Chr(34), "")
    SECURICS_AGENT_GROUP = Replace(args(15), Chr(34), "")
    ...
    If objFSO.fileExists(home_dir & "ossec.conf") Then
        ...
        If SECURICS_MANAGER <> "" or SECURICS_MANAGER_PORT <> "" or SECURICS_PROTOCOL <> "" or SECURICS_KEEP_ALIVE_INTERVAL <> "" or SECURICS_TIME_RECONNECT <> "" Then
            If SECURICS_PROTOCOL <> "" and InStr(SECURICS_PROTOCOL,",") Then
                protocol_list=Split(LCase(SECURICS_PROTOCOL),",")
            Else
                protocol_list=Array(LCase(SECURICS_PROTOCOL))
            End If
            If SECURICS_MANAGER <> "" Then
                Set re = new regexp
                re.Pattern = "\s+<server>(.|\n)+?</server>"
                If InStr(SECURICS_MANAGER,",") Then
                    ip_list=Split(SECURICS_MANAGER,",")
                Else
                    ip_list=Array(SECURICS_MANAGER)
                End If

                unique_protocol_list=get_unique_array_values(protocol_list)

                if ( UBound(protocol_list) >= UBound(ip_list) And UBound(unique_protocol_list) = 0 ) Or (SECURICS_PROTOCOL = "") Or ( UBound(unique_protocol_list) = 0 And LCase(unique_protocol_list(0)) = "tcp" ) Then
            ...
            If SECURICS_MANAGER_PORT <> "" Then ' manager server_port
                If InStr(strText, "<port>") > 0 Then
                    strText = Replace(strText, "<port>1514</port>", "<port>" & SECURICS_MANAGER_PORT & "</port>")
                End If

            End If

            If SECURICS_KEEP_ALIVE_INTERVAL <> "" Then
                If InStr(strText, "<notify_time>") > 0 Then
                    Set re = new regexp
                    re.Pattern = "<notify_time>.*</notify_time>"
                    re.Global = True
                    strText = re.Replace(strText, "<notify_time>" & SECURICS_KEEP_ALIVE_INTERVAL & "</notify_time>")
                End If
            End If

            If SECURICS_TIME_RECONNECT <> "" Then 'TODO fix the - and use _
                If InStr(strText, "<time-reconnect>") > 0 Then
                    Set re = new regexp
                    re.Pattern = "<time-reconnect>.*</time-reconnect>"
                    re.Global = True
                    strText = re.Replace(strText, "<time-reconnect>" & SECURICS_TIME_RECONNECT & "</time-reconnect>")
                End If
            End If
        End If

        If SECURICS_REGISTRATION_SERVER <> "" or SECURICS_REGISTRATION_PORT <> "" or SECURICS_REGISTRATION_PASSWORD <> "" or SECURICS_REGISTRATION_CA <> "" or SECURICS_REGISTRATION_CERTIFICATE <> "" or SECURICS_REGISTRATION_KEY <> "" or SECURICS_AGENT_NAME <> "" or SECURICS_AGENT_GROUP <> "" or ENROLLMENT_DELAY <> "" Then
            ...
            If SECURICS_REGISTRATION_SERVER <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <manager_address>" & SECURICS_REGISTRATION_SERVER & "</manager_address>"& vbCrLf &"    </enrollment>")
            End If

            If SECURICS_REGISTRATION_PORT <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <port>" & SECURICS_REGISTRATION_PORT & "</port>"& vbCrLf &"    </enrollment>")
            End If

            If SECURICS_REGISTRATION_PASSWORD <> "" Then
                Set objFile = objFSO.CreateTextFile(home_dir & "authd.pass", ForWriting)
                objFile.WriteLine SECURICS_REGISTRATION_PASSWORD
                objFile.Close
                strText = Replace(strText, "    </enrollment>", "      <authorization_pass_path>authd.pass</authorization_pass_path>"& vbCrLf &"    </enrollment>")
            End If

            If SECURICS_REGISTRATION_CA <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <server_ca_path>" & SECURICS_REGISTRATION_CA & "</server_ca_path>"& vbCrLf &"    </enrollment>")
            End If

            If SECURICS_REGISTRATION_CERTIFICATE <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_certificate_path>" & SECURICS_REGISTRATION_CERTIFICATE & "</agent_certificate_path>"& vbCrLf &"    </enrollment>")
            End If

            If SECURICS_REGISTRATION_KEY <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_key_path>" & SECURICS_REGISTRATION_KEY & "</agent_key_path>"& vbCrLf &"    </enrollment>")
            End If

            If SECURICS_AGENT_NAME <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_name>" & SECURICS_AGENT_NAME & "</agent_name>"& vbCrLf &"    </enrollment>")
            End If

            If SECURICS_AGENT_GROUP <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <groups>" & SECURICS_AGENT_GROUP & "</groups>"& vbCrLf &"    </enrollment>")
            End If
...
Public Function CheckSvcRunning()
    ...
    If svc.Count <> 0 Then
        state = wmi.Get("Win32_Service.Name='" & SERVICE & "'").State
        Session.Property("SECURICSRUNNING") = state
    End If
...
Public Function StartSecuricsSvc()
	Set WshShell = CreateObject("WScript.Shell")
    StartSvc = "NET START SecuricsSvc"
    WshShell.run StartSvc, 0, True
End Function

Public Function SetSecuricsPermissions()
    strArgs = Session.Property("CustomActionData")
    args = Split(strArgs, "/+/")

...
def init_opensearch():
    ...
    env_vars = {
        ...
        'OPENSEARCH_INITIAL_ADMIN_PASSWORD': 'SecuricsTest99$',
    }
...
def test_initialize_indexer_connector(opensearch):
    ...
    ## Remove folder queue/indexer/db/securics-states-vulnerabilities-cluster
    if Path("queue/indexer/db/securics-states-vulnerabilities-default").exists():
        for file in Path("queue/indexer/db/securics-states-vulnerabilities-default").glob("*"):
            file.unlink()
        Path("queue/indexer/db/securics-states-vulnerabilities-default").rmdir()
    ...
    while counter < 10:
        ...
        response = requests.get(url)
        if response.status_code == 200 and 'securics-states-vulnerabilities-default' in response.text:
...
def test_add_bulk_indexer_connector(opensearch):
    ...
    counter = 0
    while counter < 10:
        url = 'http://localhost:9200/securics-states-vulnerabilities-default/_search'
        query = {
            "query": {
                "match_all": {}
            }
    ...
    url = 'http://localhost:9200/securics-states-vulnerabilities-default/_delete_by_query?refresh=true'
    query = {
        "query": {
            "match_all": {}
        }
    ...
    url = 'http://localhost:9200/securics-states-vulnerabilities-cluster/_doc/000_pkghash_CVE-2022-123456?refresh=true'
    query = """{
      "agent": {
        "build": {

...
SECURICS_SERVER="../src/init/securics-server.sh"
SECURICS_AGENT="../src/init/securics-client.sh"
SECURICS_LOCAL="../src/init/securics-local.sh"
NSIS_FILE="../src/win32/securics-installer.nsi"
MSI_FILE="../src/win32/securics-installer.wxs"
FW_INIT="../framework/securics/__init__.py"
CLUSTER_INIT="../framework/securics/core/cluster/__init__.py"
...
then
    ...
    sed -E -i'' -e "s/^(VERSION=+)\"v.*\"/\1\"$version\"/" $SECURICS_SERVER
    sed -E -i'' -e "s/^(VERSION=+)\"v.*\"/\1\"$version\"/" $SECURICS_AGENT
    sed -E -i'' -e "s/^(VERSION=+)\"v.*\"/\1\"$version\"/" $SECURICS_LOCAL

    # File securics-installer.nsi
    ...
    # File securics-installer.wxs

    egrep '<Product Id="\*" Name="Securics Agent" Language="1033" Version=".+" Manufacturer=' $MSI_FILE > /dev/null
    ...
    sed -E -i'' -e "s/(<Product Id=\"\*\" Name=\"Securics Agent\" Language=\"1033\" Version=\").+(\" Manufacturer=)/\1${version:1}\2/g" $MSI_FILE
...
then
    ...
    sed -E -i'' -e "s/^(REVISION=+)\".*\"/\1\"$revision\"/" $SECURICS_SERVER
    sed -E -i'' -e "s/^(REVISION=+)\".*\"/\1\"$revision\"/" $SECURICS_AGENT
    sed -E -i'' -e "s/^(REVISION=+)\".*\"/\1\"$revision\"/" $SECURICS_LOCAL
...
for spec_file in $SPEC_FILES; do
    ...
    if [ -z "$UPDATE_RELEASE_DATE" ] ; then
        sed -E -i'' "/%changelog/a * $spec_date support <info@rvbionics.com> - ${VERSION}\n\
- More info: https://documentation.rvbionics.com/current/release-notes/release-$mayor-$minor-$patch.html" $spec_file
    else
...
for changelog_file in $CHANGELOG_FILES; do
    ...
    install_type=$(sed -E 's/.*securics-(manager|agent).*/securics-\1/' <<< $changelog_file)
    if [ -z "$UPDATE_RELEASE_DATE" ] ; then
        changelog_string="$install_type (${VERSION}-RELEASE) stable; urgency=low\n\n  * More info: https://documentation.rvbionics.com/current/release-notes/release-$mayor-$minor-$patch.html\
\n\n -- Securics, Inc <info@rvbionics.com>  $bump_date\n"
...
for copyright_file in $COPYRIGHT_FILES; do
    sed -E -i'' "s/(\sSecurics, Inc <info@rvbionics.com> on).*/\1 $bump_date/" $copyright_file
done

# MacOS pkgproj files
for pkgproj_file in $PKGPROJ_FILES; do
    sed -E -i'' "s/(<string>)([0-9]+\.){2}[0-9]+-[0-9]+(<\/string>)/\1$VERSION-1\3/" $pkgproj_file
    sed -E -i'' "s/(<string>securics-agent-)([0-9]+\.){2}[0-9]+-[0-9]+/\1$VERSION-1/" $pkgproj_file
done

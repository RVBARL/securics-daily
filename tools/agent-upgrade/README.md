...
```
openssl req -new -nodes -newkey rsa:2048 -keyout wpkcert.key -out wpkcert.csr -subj '/C=US/ST=CA/O=Securics'
```

- `/C=US` is the country.
- `/ST=CA` is the state.
- `/O=Securics` is the organization's name.
...
```
curl -Lo securics-3.0.zip https://github.com/wazuh/wazuh/archive/3.0.zip
unzip securics-3.0.zip
```

2. Compile the project:

```
make -C securics-3.0/src TARGET=agent
```
...
In this particular case, the Securics Project's root directory contains the proper `upgrade.sh` file.
...
1. Overwrite the shipped root CA with your certificate. This will prevent your agent from upgrading using WPK packages from Securics.
...
Get the WPK package into the Securics manager and run:
...
> securics_agent_v3.0.0-beta7_centos_7_x86_64.wpk

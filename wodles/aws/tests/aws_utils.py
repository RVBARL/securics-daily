...
SECURICS_VERSION = "4.5.0"

TEST_SECURICS_PATH = "/var/ossec"
TEST_DATABASE = "test"
...
def get_securics_integration_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
    ...
    """Return a dict containing every parameter supported by SecuricsIntegration. Used to simulate different ossec.conf
    configurations.
...
def get_securics_aws_database_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
                                      db_name: str = TEST_DATABASE, access_key: str = None, secret_key: str = None,
                                      iam_role_arn: str = None, region: str = None, discard_field: str = None,
                                      discard_regex: str = None, sts_endpoint: str = None, service_endpoint: str = None,
                                      iam_role_duration: str = None, external_id: str = None):
    """Return a dict containing every parameter supported by SecuricsAWSDatabase.
...
def get_mocked_securics_integration(**kwargs):
    with patch('securics_integration.SecuricsIntegration.get_client'), \
            patch('securics_integration.sqlite3.connect'), \
            patch('securics_integration.utils.find_securics_path', return_value=TEST_SECURICS_PATH), \
            patch('securics_integration.utils.get_securics_version', return_value=SECURICS_VERSION):
        return securics_integration.SecuricsIntegration(**get_securics_integration_parameters(**kwargs))


def get_mocked_securics_aws_database(**kwargs):
    with patch('securics_integration.SecuricsAWSDatabase.check_metadata_version'), \
            patch('securics_integration.SecuricsIntegration.get_client'), \
            patch('securics_integration.sqlite3.connect'), \
            patch('securics_integration.utils.find_securics_path', return_value=TEST_SECURICS_PATH), \
            patch('securics_integration.utils.get_securics_version', return_value=SECURICS_VERSION):
        return securics_integration.SecuricsAWSDatabase(**get_securics_aws_database_parameters(**kwargs))

...
def append_securics_prefixes(dictionary: Dict, path_fields: Dict[Any, List[Tuple[str, str]]]) -> None:
    """Append Securics prefix to all path fields in a dictionary.
    Parameters
    ----------
    dictionary : dict
...
def generate_self_signed_certificate(private_key: rsa.RSAPrivateKey, certificate_path: str):
    ...
    subject = issuer = x509.Name([
        ...
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"Securics"),
        x509.NameAttribute(NameOID.COMMON_NAME, u"rvbionics.com"),
    ])
...
def read_yaml_config(config_file: str = CONFIG_FILE_PATH, default_conf: dict = None) -> Dict:
    ...
    # Append Securics prefixes to all relative paths in configuration
    append_securics_prefixes(configuration, {API_SSL_PATH: [('https', 'key'), ('https', 'cert'), ('https', 'ca')]})

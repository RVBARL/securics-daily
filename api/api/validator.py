...
_securics_key = re.compile(r'[a-zA-Z0-9]+$')
_securics_version = re.compile(r'^(?:securics |)v?\d+\.\d+\.\d+$', re.IGNORECASE)
...
SECURICS_COMPONENT_CONFIGURATION_MAPPING = MappingProxyType(
    {
...
def is_safe_path(path: str, basedir: str = common.SECURICS_PATH, relative: bool = True) -> bool:
    ...
    path : str
        Path to be checked.
    basedir : str
        Securics installation directory.
    relative : bool
...
def check_component_configuration_pair(component: str, configuration: str) -> SecuricsError:
    """

    Parameters
    ----------
    component : str
        Securics component name.
    configuration : str
    ...
    -------
    SecuricsError
        It can either return a `SecuricsError` or `None`, depending on the given component and configuration. The exception
        is returned and not raised because we use the object to create a problem on API level.
    """
    if configuration not in SECURICS_COMPONENT_CONFIGURATION_MAPPING[component]:
        return SecuricsError(1128, extra_message=f"Valid configuration values for '{component}': "
                                              f"{SECURICS_COMPONENT_CONFIGURATION_MAPPING[component]}")
...
@Draft4Validator.FORMAT_CHECKER.checks("securics_key")
def format_securics_key(value):
    return check_exp(value, _securics_key)


@Draft4Validator.FORMAT_CHECKER.checks("securics_version")
def format_securics_version(value):
    return check_exp(value, _securics_version)

...
from securics.core.exception import SecuricsError, SecuricsPermissionError, SecuricsResourceNotFound, \
    SecuricsInternalError
...
@pytest.mark.parametrize('exception_type, code, extra_fields, returned_code, returned_exception', [
    ...
    (SecuricsError, 1000, ['remediation', 'code'], 400, ProblemException),
    (SecuricsPermissionError, 4000, ['remediation', 'code'], 403, ProblemException),
    (SecuricsResourceNotFound, 1710, ['remediation', 'code'], 404, ProblemException),
    (SecuricsInternalError, 1000, ['remediation', 'code'], 500, ProblemException)
])
...
@pytest.mark.parametrize('obj, code', [
    ((SecuricsError(6001), ['value0', 'value1']), 429),
    ((SecuricsInternalError(1000), ['value0', 'value1']), None),
    ((SecuricsPermissionError(4000), ['value0', 'value1']), None),
    ((SecuricsResourceNotFound(1710), ['value0', 'value1']), None)
])
...
@pytest.mark.parametrize('link', [
    '',
    'https://documentation.rvbionics.com/current/user-manual/api/reference.html'
])
@pytest.mark.asyncio
...
async def test_only_master_endpoint(mock_exc):
    ...
    with patch('api.util.running_in_master_node', return_value=False):
        await func_()
        mock_exc.assert_called_once_with(SecuricsResourceNotFound(902))

...
- Test the mapping
    Run the script with a relative path from the `securics` folder to check if a file is mapped.
    ```
    python3 _test_mapping.py framework/securics/agent.py
    ```
    ...
    > **NOTE:** Please use a relative path when trying the testing mode. For instance: `/home/securics/Desktop/git/securics/framework/securics/agent.py` -> `framework/securics/agent.py`

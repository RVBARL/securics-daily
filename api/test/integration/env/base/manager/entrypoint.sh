...
cp -rf /tmp_volume/config/* /var/ossec/ && chown -R securics:securics /var/ossec/api
...
if [ $4 == "standalone" ]; then
  ...
  sed -i "s:validate_responses=False:validate_responses=True:g" /var/ossec/api/scripts/securics_apid.py
fi
...
chown root:securics /var/ossec/etc/client.keys
chown -R securics:securics /var/ossec/queue/db
chown -R securics:securics /var/ossec/etc/shared
chmod --reference=/var/ossec/etc/shared/default /var/ossec/etc/shared/group*
cd /var/ossec/etc/shared && find -name merged.mg -exec chown securics:securics {} \; && cd /
chown root:securics /var/ossec/etc/shared/ar.conf
...
/var/ossec/bin/securics-control start
...
if [ "$3" == "master" ]; then
  ...
  # Wait until Securics API is ready
  elapsed_time=0

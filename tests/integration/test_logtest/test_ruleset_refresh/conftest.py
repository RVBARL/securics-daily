...
def configure_rules_list(test_metadata):
    ...
    Restarting Securics is not needed for applying the configuration, it is optional.
    """

    # configuration for testing
    rules_dir = os.path.join(SECURICS_PATH, test_metadata['rule_dir'])
...
def configure_cdbs_list(test_metadata):
    ...
    cdb_dir = os.path.join(SECURICS_PATH, test_metadata['cdb_dir'])
    ...
    rule_dir = os.path.join(SECURICS_PATH, test_metadata['rule_dir'])
...
def configure_decoders_list(test_metadata):
    ...
    Restarting Securics is needed for applying the configuration, it is optional.
    """

    # configuration for testing
    decode_dir = os.path.join(SECURICS_PATH, test_metadata['decoder_dir'])

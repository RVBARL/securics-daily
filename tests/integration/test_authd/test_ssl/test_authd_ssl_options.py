...
brief: These tests will check if the 'SSL' (Secure Socket Layer) protocol-related settings of
       the 'securics-authd' daemon are working correctly. The 'securics-authd' daemon can
       automatically add a Securics agent to a Securics manager and provide the key
...
def test_ossec_auth_configurations(test_configuration, test_metadata, set_securics_configuration,
                                   truncate_monitored_files, daemons_handler,
                                   configure_sockets_environment, wait_for_authd_startup):
    '''
    description:
        Checks if the 'SSL' settings of the 'securics-authd' daemon work correctly by enrolling agents

...
brief: These tests will check if the 'remote enrollment' option of the 'securics-authd' daemon
       settings is working properly. The 'securics-authd' daemon can automatically add
       a Securics agent to a Securics manager and provide the key to the agent.
...
references:
    - https://documentation.rvbionics.com/current/user-manual/reference/ossec-conf/auth.html#remote-enrollment
...
from securics_testing.constants.ports import DEFAULT_SSL_CLUSTER_PORT
from securics_testing.utils import callbacks
from securics_testing.modules.authd import PREFIX
from securics_testing.tools.socket_controller import SocketController
...
from securics_testing.constants.daemons import AUTHD_DAEMON, SECURICS_DB_DAEMON, MODULES_DAEMON
...
monitored_sockets_params = [(MODULES_DAEMON, None, True), (SECURICS_DB_DAEMON, None, True), (AUTHD_DAEMON, None, True)]
...
def test_remote_enrollment(test_configuration, test_metadata, set_securics_configuration,
                           truncate_monitored_files, daemons_handler):
    '''
    description:
        Checks if the 'securics-authd' daemon remote enrollment is enabled/disabled according
    ...
    parameters:
        ...
        - daemons_handler:
            type: fixture
            brief: Restarts securics or a specific daemon passed.
    ...
    expected_output:
        - r'Accepting connections on port 1515. No password required.' (When the 'securics-authd' daemon)
    ...
    file_monitor.FileMonitor(SECURICS_LOG_PATH).start(timeout=5,

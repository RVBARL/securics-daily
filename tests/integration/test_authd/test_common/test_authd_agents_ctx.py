...
brief: These tests will check if the 'securics-authd' daemon correctly handles the enrollment requests
       from agents with pre-existing IP addresses or names. The 'securics-authd' daemon can automatically
       add a Securics agent to a Securics manager and provide the key to the agent. It is used along with
...
references:
    - https://documentation.rvbionics.com/current/user-manual/reference/daemons/securics-authd.html
    - https://documentation.rvbionics.com/current/user-manual/reference/tools/agent_groups.html
...
import pytest
import time
from securics_testing.constants.paths.sockets import AUTHD_SOCKET_PATH
from securics_testing.constants.ports import DEFAULT_SSL_REMOTE_ENROLLMENT_PORT
from securics_testing.utils.agent_groups import check_agent_groups
from securics_testing.utils.db_queries.global_db import delete_agent
from securics_testing.utils.client_keys import check_client_keys
...
def test_ossec_authd_agents_ctx(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files,
                                clean_agents_ctx, daemons_handler, wait_for_authd_startup, connect_to_sockets, set_up_groups):
    '''
    description:
        Check if when the 'securics-authd' daemon receives an enrollment request from an agent
    ...
    input_description:
        Different test cases are contained in an external YAML file (securics_conf.yaml)
        which includes configuration settings for the 'securics-authd' daemon.

    expected_output:
        - r'Accepting connections on port 1515' (When the 'securics-authd' daemon is ready to accept enrollments)

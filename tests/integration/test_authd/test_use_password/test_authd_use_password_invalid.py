...
def test_authd_use_password_invalid(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files_module,
    ...
    assertions:
        ...
        - securics-manager.service must not be able to restart.

    input_description:
        ./data/configuration_template/config_authd_use_password_invalid.yaml: Securics config needed for the tests.
    ...
    if log == 'Invalid password provided.':
        pytest.xfail(reason="No password validation in authd.pass - Issue securics/securics#16282.")

    # Verify securics-manager fails at restart.

...
from securics_testing.constants.paths.configurations import CUSTOM_RULES_PATH, CUSTOM_RULES_FILE, SECURICS_CONF_PATH
from securics_testing.constants.paths.logs import ALERTS_JSON_PATH, SECURICS_LOG_PATH
from securics_testing.constants.users import SECURICS_UNIX_GROUP, SECURICS_UNIX_USER
from securics_testing.modules.analysisd import patterns
from securics_testing.tools.monitors import file_monitor
from securics_testing.utils import callbacks, file


@pytest.fixture()
def prepare_custom_rules_file(request, test_metadata):
    """Configure a syscollector custom rules for testing.
    Restarting securics-analysisd is required to apply this changes.
    """
    ...
    shutil.chown(target_rule, SECURICS_UNIX_USER, SECURICS_UNIX_GROUP)
...
def configure_local_rules(request, test_configuration):
    """Configure a custom rule for testing. Restart Securics is needed for applying the configuration. """
...
def configure_remove_tags(request, test_metadata):
    """Configure a custom settting for testing. Restart Securics is needed for applying the configuration. """
    # Remove test case tags from ossec.conf
    file.replace_regex_in_file(test_metadata['remove_tags'], [''] * len(test_metadata['remove_tags']), SECURICS_CONF_PATH)

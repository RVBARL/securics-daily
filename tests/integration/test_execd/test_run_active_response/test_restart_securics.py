...
brief: Active responses execute a script in response to the triggering of specific alerts based
       ...
       which are executed by the 'securics-execd' daemon via scripts, run correctly.
...
from securics_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, SECURICS_LOG_PATH
from securics_testing.constants.platforms import WINDOWS
from securics_testing.modules.execd.active_response.patterns import ACTIVE_RESPONSE_RESTART_SECURICS
from securics_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from securics_testing.modules.execd.configuration import EXECD_DEBUG
from securics_testing.modules.execd.patterns import EXECD_SHUTDOWN_RECEIVED
...
ar_conf = 'restart-securics0 - restart-securics - 0\nrestart-securics0 - restart-securics.exe - 0'
...
def test_execd_restart(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                       set_securics_configuration, configure_ar_conf, remoted_simulator, authd_simulator,
                       daemons_handler, send_execd_message):
    '''
    description: Check if 'restart-securics' command of 'active response' is executed correctly.
                 For this purpose, a simulated server is used, from which the active response is sent.
                 This response includes the order to restart the Securics agent, which must restart after
    ...
    assertions:
        ...
        - Check the restart-securics program is used.
    input_description:
    ...
    if error_message := test_metadata.get('expected_error'):
        ...
        assert ar_monitor.callback_result, 'AR `securics-restart` did not fail.'
        return

    securics_log_monitor.start(callback=generate_callback(EXECD_SHUTDOWN_RECEIVED))
    assert securics_log_monitor.callback_result, 'Execd `shutdown` log not raised.'

    ar_monitor.start(callback=generate_callback(ACTIVE_RESPONSE_RESTART_SECURICS))
    assert ar_monitor.callback_result, 'AR `restart-securics` program not used.'

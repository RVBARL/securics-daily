...
brief: Integratord manages Securics integrations with other applications such as Yara or Slack, by feeding
...
references:
    - https://documentation.rvbionics.com/current/user-manual/manager/manual-integration.html#slack
    - https://documentation.rvbionics.com/current/user-manual/reference/daemons/securics-integratord.html
...
from securics_testing.constants.daemons import ANALYSISD_DAEMON, SECURICS_DB_DAEMON, INTEGRATOR_DAEMON
from securics_testing.constants.paths.logs import ALERTS_JSON_PATH, SECURICS_LOG_PATH
from securics_testing.modules.analysisd.configuration import ANALYSISD_DEBUG
from securics_testing.modules.integratord.configuration import INTEGRATORD_DEBUG
from securics_testing.modules.integratord.patterns import INTEGRATORD_THIRD_PARTY_RESPONSE, INTEGRATORD_INODE_CHANGED, \
...
from securics_testing.utils.commands import run_local_command_returning_output
from securics_testing.utils.configuration import get_test_cases_data, load_configuration_template
from securics_testing.utils.file import copy, remove_file
...
daemons_handler_configuration = {'daemons': [INTEGRATOR_DAEMON, SECURICS_DB_DAEMON, ANALYSISD_DAEMON]}
...
def test_integratord_change_json_inode(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files,
                                       configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    description: Check that securics-integratord detects a change in the inode of the alerts.json and continues reading
                 alerts.

    test_phases:
        - setup:
            - Apply ossec.conf configuration changes according to the configuration template and use case.
            - Truncate Securics's logs.
        ...
        - test:
            ...
            - Replace the `alerts.json` file while securics-integratord is reading it.
            - Wait for the inode change to be detected by securics-integratord.
            - Check if securics-integratord detects that the file's inode has changed.
            - Insert an alert in the `alerts.json` file.
    ...
    parameters:
        ...
        - set_securics_configuration:
            type: fixture
            brief: Set securics configuration.
    ...
    expected_output:
        - r'.+securics-integratord.*DEBUG: jqueue_next.*Alert file inode changed.*'
        - r'.+securics-integratord.*Processing alert.*'
        - r'.+securics-integratord.*<Response [200]>'
    '''
    securics_monitor = FileMonitor(SECURICS_LOG_PATH)
    ...
    # Start monitor
    securics_monitor.start(callback=generate_callback(INTEGRATORD_THIRD_PARTY_RESPONSE), timeout=session_parameters.default_timeout)

    # Check that expected log appears
    assert securics_monitor.callback_result
    ...
    # Start monitor
    securics_monitor.start(callback=generate_callback(INTEGRATORD_INODE_CHANGED), timeout=session_parameters.default_timeout)
...
def test_integratord_read_valid_alerts(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files,
                                       configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    ...
    expected_output:
        - r'.+securics-integratord.*alert_id.*\"integration\": \"slack\".*'
    '''
...
def test_integratord_read_invalid_alerts(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files,
                                         configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    ...
    test_phases:
        ...
        - test:
            ...
            - Check if securics-integratord process the alert and report an error.
        - teardown:
            - Truncate Securics's logs.
            - Restore initial configuration, both `ossec.conf` and `local_internal_options.conf`.
    ...
    expected_output:
        - r'.+securics-integratord.*WARNING: Invalid JSON alert read.*'
        - r'.+securics-integratord.*WARNING: Overlong JSON alert read.*'
    ...
    # Start monitor
    securics_monitor.start(callback=generate_callback(callback), timeout=session_parameters.default_timeout)

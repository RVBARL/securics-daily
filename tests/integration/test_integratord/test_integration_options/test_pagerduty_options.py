...
brief: Integratord manages securics integrations with other applications such as Slack, Pagerduty, Shuffle, Yara or
...
references:
    - https://documentation.rvbionics.com/current/user-manual/capabilities/virustotal-scan/integration.html
    - https://documentation.rvbionics.com/current/user-manual/reference/daemons/securics-integratord.htm
...
from securics_testing.modules.integratord.patterns import INTEGRATORD_THIRD_PARTY_RESPONSE, INTEGRATORD_ENABLED_INTEGRATION, \
...
def test_pagerduty_no_option_tag(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files,
    ...
    parameters:
        ...
        - daemons_handler:
            type: fixture
            brief: Restart securics daemon before starting a test.
    ...
    # Start monitor
    securics_monitor.start(callback=generate_callback(INTEGRATORD_ENABLED_INTEGRATION,
    ...
    # Start monitor
    securics_monitor.start(callback=generate_callback(INTEGRATORD_OPTIONS_FILE_DOES_NOT_EXISTENT), timeout=session_parameters.default_timeout)

    # Check no options JSON file is detected
    assert securics_monitor.callback_result

    # Start monitor
    securics_monitor.start(callback=generate_callback(INTEGRATORD_SENDING_MESSAGE,
...
def test_pagerduty_options(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files,
                           configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    ...
    if not test_metadata['sends_message']:
        # Start monitor
        securics_monitor.start(callback=generate_callback(INTEGRATORD_ERROR_RUNNING_INTEGRATION,
                                                    replacement={"integration": 'pagerduty'}),
                                                    timeout=session_parameters.default_timeout)
        ...
        # Start monitor
        message = securics_monitor.start(callback=generate_callback(INTEGRATORD_SENDING_MESSAGE,
                                                    replacement={"integration": 'PagerDuty'}),
                                                    timeout=session_parameters.default_timeout,

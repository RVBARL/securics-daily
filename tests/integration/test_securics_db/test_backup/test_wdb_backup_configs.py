...
brief: Securics-db is the daemon in charge of the databases with all the Securics persistent information, exposing a socket
       to receive requests and provide information. Securics-db has the capability to do automatic database backups, based
...
from securics_testing.constants.paths.logs import SECURICS_PATH, SECURICS_LOG_PATH
from securics_testing.utils.time import validate_interval_format, time_to_seconds
from securics_testing.modules.securics_db import patterns
...
# Configuration
t_config_path = Path(CONFIGURATIONS_FOLDER_PATH, 'configuration_securics_db_backups_conf.yaml')
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_securics_db_backups_conf.yaml')
...
backups_path = Path(SECURICS_PATH, 'backup', 'db')
...
def test_wdb_backup_configs(test_configuration, test_metadata, set_securics_configuration,
    ...
    description: Check that given different wdb backup configuration parameters, the expected behavior is achieved.
                 For this, the test gets a series of parameters for the securics_db_backups_conf.yaml file and applies
    ...
    input_description:
        - Test cases are defined in the parameters and metada variables, that will be applied to the the
          securics_db_backup_command.yaml file. The parameters tested are: "enabled", "interval" and "max_files".
    ...
    try:
        ...
        if not validate_interval_format(test_interval):
            securics_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_INTERVAL_CALLBACK), timeout=timeout)
            assert securics_log_monitor.callback_result, 'Did not receive expected ' \
                                                    '"Invalid value element for interval..." event'

            return
        elif not isinstance(test_max_files, numbers.Number) or test_max_files==0:
            securics_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_MAX_FILES_CALLBACK), timeout=timeout)
            assert securics_log_monitor.callback_result, 'Did not receive expected ' \
    ...
    # Manage if backup generation is enabled - one or more backups expected
        ...
        securics_log_monitor.start(callback=callbacks.generate_callback(patterns.BACKUP_CREATION_CALLBACK),
                                        timeout=timeout, accumulations=int(test_max_files)+1)
        result = securics_log_monitor.callback_result

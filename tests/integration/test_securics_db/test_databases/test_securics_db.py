...
from securics_testing.utils.callbacks import make_callback
from securics_testing.modules.securics_db import SECURICS_DB_PREFIX
from securics_testing.utils import configuration
from securics_testing.utils.database import query_wdb
from securics_testing.utils.db_queries.agent_db import agent_integrity_check
...
# List items -> (securics_daemon: str,(
#                mitm: ManInTheMiddle
#                daemon_first: bool))
# Example1 -> ('securics-clusterd', None)              Only start securics-clusterd with no MITM
# Example2 -> ('securics-clusterd', (my_mitm, True))   Start MITM and then securics-clusterd
...
def validate_securics_db_response(expected_output, response):
    """ Method to validate the Securics-DB response.
...
def test_securics_db_messages_agent(test_metadata, configure_sockets_environment_module, connect_to_sockets_module,
                                 clean_databases, clean_registered_agents, insert_agents_test):
    '''
    description: Check that every input agent message in securics-db socket generates the proper output to securics-db
    ...
    parameters:
        ...
        - clean_registered_agents:
            type: fixture
            brief: Remove all agents of securicsdb.
        - insert_agents_test:
            type: fixture
    ...
    for index, stage in enumerate(test_metadata['test_case']):
        ...
        if 'use_regex' in stage and stage['use_regex'] == 'yes':
            ...
            match = validate_securics_db_response(expected_output, response)
...
def test_securics_db_messages_global(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in securics-db socket generates the proper output to securics-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
                 stages (test_case, input field) and compare the result with the input list of stages (test_case,
                 output field).
...
def test_securics_db_range_checksum(configure_sockets_environment_module, connect_to_sockets_module,
    ...
    log_monitor.start(callback=make_callback('range checksum: Time: ', prefix=SECURICS_DB_PREFIX,
                                             escape=True), timeout=SECURICS_DB_CHECKSUM_CALCULUS_TIMEOUT)
    ...
    agent_integrity_check()
    log_monitor.start(callback=make_callback('range checksum avoided', prefix=SECURICS_DB_PREFIX,
                                             escape=True), timeout=SECURICS_DB_CHECKSUM_CALCULUS_TIMEOUT)
...
def test_securics_db_messages_tasks(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in securics-db socket generates the proper output to securics-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of

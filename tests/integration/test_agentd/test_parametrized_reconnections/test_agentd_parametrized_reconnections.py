...
brief: The 'securics-agentd' program is the client-side daemon that communicates with the server.
       The objective is to check how the 'securics-agentd' daemon behaves when there are delays
       between connection attempts to the 'securics-remoted' daemon using TCP and UDP protocols.
       The 'securics-remoted' program is the server side daemon that communicates with the agents.
...
references:
    - https://documentation.rvbionics.com/current/user-manual/registering/index.html
...
from securics_testing.modules.agentd.configuration import AGENTD_DEBUG, AGENTD_WINDOWS_DEBUG, AGENTD_TIMEOUT
from securics_testing.modules.agentd.patterns import AGENTD_TRYING_CONNECT, AGENTD_CONNECTED_TO_SERVER
...
def test_agentd_parametrized_reconnections(test_metadata, set_securics_configuration, configure_local_internal_options,
    ...
    assertions:
        - Verify that when the 'securics-agentd' daemon initializes, it connects to
          the 'securics-remoted' daemon of the manager before reaching the maximum number of attempts.
    ...
    securics_log_monitor.start(accumulations = test_metadata['MAX_RETRIES'], callback=callbacks.generate_callback(AGENTD_TRYING_CONNECT,{'IP':'','PORT':''}))
    assert (securics_log_monitor.callback_result != None), f'Trying to connect to server message not found expected times'

    #Check number of connected message is the expected
    if test_metadata['ENROLL'] == 'yes':
        securics_log_monitor.start(callback=callbacks.generate_callback(AGENTD_CONNECTED_TO_SERVER))

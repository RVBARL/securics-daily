...
brief: The 'securics-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the Securics component (agent or manager) starts when
       the 'location' tag is set in the configuration, and the Securics API returns the same values for
...
references:
    ...
    - https://documentation.rvbionics.com/current/user-manual/reference/ossec-conf/localfile.html
...
from securics_testing.utils.file import write_json_file
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                reset_ofe_status, remove_all_localfiles_securics_config, set_securics_configuration, pre_send_journal_logs,
                                daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'securics-logcollector' daemon starts properly when the 'journald' tag is used 
                 ...
                 Finally, the test will verify that the Securics-logcollector read the logs, and the Securics API returns the correct values
    ...
    parameters:
        ...
        - remove_all_localfiles_securics_config:
            type: fixture
            brief: Remove all 'localfile' sections from the Securics configuration.
        - reset_ofe_status:
            type: fixture
    ...
    assertions:
        - Verify that the Securics component (agent or manager) can start when the 'jouranld' tag is used.
        - Verify that the Securics component (agent or manager) can read the logs from the 'journald' log format and the logs older than the current logcollector start time.

    input_description: A configuration file with journal block settings and the expected log messages.
                       Those include configuration settings for `journal` configuration in 'securics-logcollector'.

    expected_output:
        - Boolean values to indicate the state of the Securics component.

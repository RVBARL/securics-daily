...
brief: The 'securics-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       correctly executed by the logcollector, and the Securics API returns the same values for
...
references:
    ...
    - https://documentation.rvbionics.com/current/user-manual/reference/ossec-conf/localfile.html#command
...
config_path = Path(CONFIGURATIONS_PATH, 'securics_basic_configuration_command.yaml')
...
def test_configuration_command(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'securics-logcollector' daemon can monitor commands that use multiple parameters.
    ...
    securics_log_monitor.start(callback=callback)

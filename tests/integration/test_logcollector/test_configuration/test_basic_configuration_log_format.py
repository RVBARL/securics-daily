...
brief: The 'securics-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'log_format' tag and the Securics API returns the same values for the configured
...
references:
    ...
    - https://documentation.rvbionics.com/current/user-manual/reference/ossec-conf/localfile.html#log-format
    - https://documentation.rvbionics.com/current/user-manual/reference/ossec-conf/localfile.html#location
...
from securics_testing.constants.paths.logs import SECURICS_LOG_PATH, MACOS_LOG_COMMAND_PATH
...
from securics_testing.modules.logcollector import patterns, PREFIX
...
default_config_path = Path(CONFIGURATIONS_PATH, 'securics_basic_configuration_log_format.yaml')
default_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format.yaml')

macos_duplicated_config_path = Path(CONFIGURATIONS_PATH, 'securics_duplicated_macos_configuration.yaml')
macos_duplicated_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_duplicated.yaml')

macos_no_defined_config_path = Path(CONFIGURATIONS_PATH, 'securics_no_defined_location_macos_configuration.yaml')
macos_no_defined_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_no_defined.yaml')

win_config_path = Path(CONFIGURATIONS_PATH, 'securics_basic_configuration_log_format_location.yaml')
...
def check_log_format_valid(test_configuration, test_metadata):
    """Check if Securics run correctly with the specified log formats.
    ...
    if test_metadata['log_format'] not in log_format_not_print_analyzing_info:
        securics_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE,
                                                   {'file': test_metadata['location']}))
        assert (securics_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_FILE
        ...
        assert (securics_log_monitor.callback_result != None), patterns.ERROR_DJB_MULTILOG_NOT_PRODUCED
        ...
        if 'location' in test_metadata and test_metadata['location'] != 'macos':
            ...
            assert (securics_log_monitor.callback_result != None), patterns.ERROR_INVALID_MACOS_VALUE
        if 'location' not in test_metadata:
            securics_log_monitor.start(timeout=5,
                                    callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MISSING_LOCATION))
            assert (securics_log_monitor.callback_result != None), patterns.ERROR_MISSING_LOCATION_VALUE

        securics_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                   {'command_path': MACOS_LOG_COMMAND_PATH}))
        assert (securics_log_monitor.callback_result != None), patterns.ERROR_MACOS_LOG_NOT_PRODUCED


def check_log_format_invalid(test_metadata):
    """Check if Securics fails because a invalid frequency configuration value.
    ...
    securics_log_monitor.start(timeout=5, callback=log_callback)
...
def check_log_file_duplicated():
    """Check if Securics shows a warning message when the configuration is duplicated."""
    securics_log_monitor = file_monitor.FileMonitor(SECURICS_LOG_PATH)
    securics_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_LOG_FILE_DUPLICATED))
    assert (securics_log_monitor.callback_result != None), patterns.ERROR_LOG_FILE_DUPLICATED
    securics_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                                 {'command_path': MACOS_LOG_COMMAND_PATH}))
    assert (securics_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_MACOS
...
def test_log_format(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'securics-logcollector' daemon detects invalid configurations for the 'log_format' tag.
                 ...
                 Finally, the test will verify that the Securics API returns the same values for the 'localfile' section
    ...
    input_description: A configuration templates (test_basic_configuration_log_format) are contained in externals
                       YAML files (securics_basic_configuration.yaml, securics_duplicated_macos_configuration.yaml, and
                       securics_no_defined_location_macos_configuration.yaml). Those templates are combined with
                       different test cases defined in the module. Those include configuration settings for
                       the 'securics-logcollector' daemon.

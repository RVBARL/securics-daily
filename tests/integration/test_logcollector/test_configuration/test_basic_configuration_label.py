...
brief: The 'securics-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'label' tag is set in the configuration, and the Securics API returns the same values for
...
references:
    ...
    - https://documentation.rvbionics.com/current/user-manual/reference/ossec-conf/localfile.html#label
...
config_path = Path(CONFIGURATIONS_PATH, 'securics_basic_configuration_label.yaml')
...
def test_configuration_label(test_configuration, test_metadata, set_securics_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'securics-logcollector' daemon can monitor log files configured to use labels.
                 ...
                 Finally, the test will verify that the Securics API returns the same values for
    ...
    securics_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE, {

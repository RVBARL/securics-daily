...
# Configuration
default_config_path = Path(CONFIGURATIONS_PATH, 'securics_basic_configuration_log_format_location.yaml')
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                set_securics_configuration, daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'securics-logcollector' daemon starts properly when the 'location' tag is used.
                 ...
                 Securics component is started by checking its process, and the Securics API returns the same
    ...
    assertions:
        - Verify that the Securics component (agent or manager) can start when the 'location' tag is used.
        - Verify that the Securics API returns the same value for the 'localfile' section as the configured one.
    ...
    if sys.platform == WINDOWS:
        if test_metadata['location'] == 'invalidchannel':
            ...
            assert (securics_log_monitor.callback_result != None), patterns.ERROR_EVENTCHANNEL
    else:

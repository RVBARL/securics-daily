...
from securics_testing.modules.logcollector.patterns import LOGCOLLECTOR_MODULE_START
...
from securics_testing.utils.file import truncate_file, replace_regex_in_file, write_json_file

# Logcollector internal paths
LOGCOLLECTOR_OFE_PATH = path_join(SECURICS_PATH, 'queue', 'logcollector', 'file_status.json')

@pytest.fixture()
def stop_logcollector(request):
    """Stop securics-logcollector and truncate logs file."""
...
@pytest.fixture()
def remove_all_localfiles_securics_config(request):
    ...
    replace_regex_in_file(list_tags, [''] * len(list_tags), SECURICS_CONF_PATH, True)

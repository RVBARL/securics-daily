...
from securics_testing.modules.fim.patterns import EMPTY_DIRECTORIES_TAG
...
def test_empty_directories_tag(test_configuration, test_metadata, set_securics_configuration, configure_local_internal_options,
                               truncate_monitored_files, daemons_handler):
    '''
    description: Check if the 'securics-syscheckd' daemon shows a debug message when an empty 'directories' tag is found.
    ...
    assertions:
        - Verify that the 'securics-syscheckd' daemon generates a debug log when
    ...
    input_description: The test cases are contained in external YAML file (cases_empty_directories_tag.yaml)
                       which includes configuration parameters for the 'securics-syscheckd' daemon and testing
    ...
    # Assert
    securics_log_monitor.start(generate_callback(EMPTY_DIRECTORIES_TAG))

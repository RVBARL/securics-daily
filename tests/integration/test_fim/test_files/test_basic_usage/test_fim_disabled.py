...
from securics_testing.modules.fim.patterns import SENDING_FIM_EVENT
...
def test_fim_disabled(test_configuration, test_metadata, set_securics_configuration, truncate_monitored_files,
                      configure_local_internal_options, folder_to_monitor, daemons_handler):

    '''
    description: Check if the 'securics-syscheckd' daemon generates FIM events when it is disabled
    ...
    assertions:
        - Verify that when the 'securics-syscheckd' daemon is disabled, no FIM events are generated.
    ...
    securics_log_monitor.start(generate_callback(SENDING_FIM_EVENT))

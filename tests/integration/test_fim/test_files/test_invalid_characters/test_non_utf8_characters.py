...
from securics_testing.modules.fim.patterns import IGNORING_DUE_TO_INVALID_NAME, SYNC_INTEGRITY_MESSAGE, EVENT_TYPE_ADDED
from securics_testing.modules.fim.utils import create_registry
from securics_testing.modules.monitord.configuration import MONITORD_ROTATE_LOG
from securics_testing.modules.fim import configuration
...
def test_nonUTF8(test_configuration, test_metadata, set_securics_configuration, configure_local_internal_options,
                  truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring):
    '''
    description: Check if the 'securics-syscheckd' is able to correctly detect a pathname containing an invalid
                 character, preventing its processing and writing a log warning.

    securics_min_version: 4.9.0
    ...
    input_description: The test cases are contained in external YAML file (cases_nonUTF8.yaml) which includes
                       configuration parameters for the 'securics-syscheckd' daemon and testing directories to monitor.
...
if sys.platform == WINDOWS:
    ...
    def test_invalid_registry(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                              set_securics_configuration, daemons_handler, detect_end_scan):
        '''
        description: Check if the 'securics-syscheckd' is able to correctly process a registry key name containing an invalid
                     character.

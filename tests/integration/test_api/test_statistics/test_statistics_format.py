...
brief: These tests will check if the stats can be obtained from the API and if they follow the expected schema.
       The Securics API is an open source 'RESTful' API that allows for interaction with the Securics manager from
...
references:
    - https://documentation.rvbionics.com/current/user-manual/api/reference.html (Get Securics daemon stats)
    - https://documentation.rvbionics.com/current/user-manual/api/reference.html (Get Securics daemon stats from an agent)
...
from securics_testing import DATA_PATH
from securics_testing.constants.api import DAEMONS_STATS_ROUTE
from securics_testing.modules.api.utils import get_base_url, login, validate_statistics
...
@pytest.mark.parametrize('test_configuration,test_metadata', zip(test1_configuration, test1_metadata),
                         ids=test1_cases_ids)
def test_manager_statistics_format(test_configuration, test_metadata, load_securics_basic_configuration,

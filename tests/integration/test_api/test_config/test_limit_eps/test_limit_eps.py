"""
copyright: Copyright (C) 2023-2024, RV Bionics Group SpA.
...
references:
    ...
    - https://documentation.rvbionics.com/current/user-manual/api/configuration.html
...
from securics_testing.constants.api import CONFIGURATION_TYPES, MANAGER_CONFIGURATION_ROUTE
from securics_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from securics_testing.modules.api.utils import login, get_base_url
from securics_testing.utils.configuration import get_test_cases_data, load_configuration_template, get_securics_conf
...
def test_limit_eps(test_configuration, test_metadata, set_securics_configuration, add_configuration,
    ...
    securics_config = " ".join(get_securics_conf())
    assert f"<maximum>{test_metadata['expected_max_value']}</maximum>" in securics_config
    assert f"<timeframe>{test_metadata['expected_timeframe_value']}</timeframe>" in securics_config

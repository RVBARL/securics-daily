...
references:
    - https://documentation.rvbionics.com/current/user-manual/api/configuration.html#logs
...
from securics_testing.constants.api import CONFIGURATION_TYPES, SECURICS_API_USER, LOGIN_ROUTE
from securics_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from securics_testing.constants.paths.logs import SECURICS_API_JSON_LOG_FILE_PATH, SECURICS_API_LOG_FILE_PATH
from securics_testing.modules.api.patterns import API_TIMEOUT_ERROR_MSG, API_LOGIN_REQUEST_MSG
...
def test_logs_formats(test_configuration, test_metadata, add_configuration, truncate_monitored_files, daemons_handler):
    ...
    if 'json' in current_formats:
        json_file_monitor = file_monitor.FileMonitor(SECURICS_API_JSON_LOG_FILE_PATH)
        if current_level == 'error':
    ...
    if 'plain' in current_formats:
        plain_file_monitor = file_monitor.FileMonitor(SECURICS_API_LOG_FILE_PATH)
        if current_level == 'error':

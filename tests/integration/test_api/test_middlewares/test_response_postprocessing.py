...
brief: These tests will check if the response_postprocessing middleware of the API handled by the 'securics-apid' daemon is
       working properly. The Securics API is an open source 'RESTful' API that allows the interaction with the Securics
...
from securics_testing.constants.api import SECURICS_API_PROTOCOL
from securics_testing.modules.api.utils import login, get_base_url, set_authorization_header
...
def test_response_postprocessing(test_configuration, test_metadata, truncate_monitored_files, daemons_handler,
    ...
    if use_login_token:
        ...
        session.mount(f"{SECURICS_API_PROTOCOL}://", adapter)

#!/bin/bash
set -e
# Program to build and package OSX securics-agent
# Securics package generator
...
ARCH="intel64"
SECURICS_SOURCE_REPOSITORY="https://github.com/wazuh/wazuh"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $SECURICS_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in Securics's installation. This also includes library files.
...
function get_pkgproj_specs() {
    ...
    pkg_file="${SECURICS_PACKAGES_PATH}/specs/securics-agent-${ARCH}.pkgproj"
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        SECURICS_PATH="${SOURCES_DIRECTORY}/securics"
        git clone --depth=1 -b ${BRANCH_TAG} ${SECURICS_SOURCE_REPOSITORY} "${SECURICS_PATH}"
    else
        SECURICS_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${SECURICS_PATH}" && git rev-parse --short HEAD)"

    export CONFIG="${SECURICS_PATH}/etc/preloaded-vars.conf"
    SECURICS_PACKAGES_PATH="${SECURICS_PATH}/packages/macos"
    AGENT_PKG_FILE="${SECURICS_PACKAGES_PATH}/package_files/securics-agent-${ARCH}.pkgproj"
    ENTITLEMENTS_PATH="${SECURICS_PACKAGES_PATH}/entitlements.plist"

    VERSION=$(cat ${SECURICS_PATH}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="securics-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="securics-agent-${VERSION}-${REVISION}.${ARCH}"
    fi
    ...
    if [ -d "${INSTALLATION_PATH}" ]; then
        ...
        echo "\nThe securics agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${SECURICS_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${SECURICS_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    if packagesbuild ${AGENT_PKG_FILE} --build-folder "${DESTINATION}/" ; then
        echo "The securics agent package for macOS has been successfully built."
        pkg_name+=".pkg"
        sign_pkg

...
# Modified by Santiago Bassett (http://www.rvbionics.com) - Feb 2016
...
if [ -z "${upgrade}" ]; then
    echo "Generating Securics configuration for a fresh installation."
    ...
    if [ -f "${INSTALLATION_SCRIPTS_DIR}/gen_ossec.sh" ]; then
        ...
        chown root:securics ${DIR}/etc/ossec.conf
...
if [[ $(dscl . -read /Groups/ossec) ]]; then
    echo "Changing group from Ossec to Securics"
    find ${DIR}/ -group ossec -user root -exec chown root:securics {} \ > /dev/null 2>&1 || true
    if [[ $(dscl . -read /Users/ossec) ]]; then
        echo "Changing user from Ossec to Securics"
        find ${DIR}/ -group ossec -user ossec -exec chown securics:securics {} \ > /dev/null 2>&1 || true
...
if [ -n "${upgrade}" ] && [ -n "${restart}" ]; then
    echo "Restarting Securics..."
    ${DIR}/bin/securics-control restart
fi

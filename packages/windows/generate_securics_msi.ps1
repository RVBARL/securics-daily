...
param (
    [string]$MSI_NAME = "securics-agent.msi",
    [string]$SIGN = "no",
...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows Securics agent msi package.

    PARAMETERS TO BUILD SECURICS-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * SECURICS:
          $ ./generate_securics_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}

            Build a devel msi:    $ ./generate_securics_msi.ps1 -MSI_NAME securics-agent_4.9.0-0_windows_0ceb378.msi -SIGN no
            Build a prod msi:     $ ./generate_securics_msi.ps1 -MSI_NAME securics-agent-4.9.0-1.msi -SIGN yes
    "
    Exit
}
...
function BuildSecuricsMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        & $SIGNTOOL_EXE sign /a /tr http://timestamp.digicert.com /fd SHA256 /td SHA256 "..\securics_modules\syscollector\build\bin\syscollector.dll"
        & $SIGNTOOL_EXE sign /a /tr http://timestamp.digicert.com /fd SHA256 /td SHA256 "..\syscheckd\build\bin\libfimdb.dll"
    }

    Write-Host "Building MSI installer..."

    & $CANDLE_EXE -nologo .\securics-installer.wxs -out "securics-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\securics-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension

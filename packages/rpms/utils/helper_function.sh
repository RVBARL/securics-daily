...
setup_build(){
    ...
    cp ${specs_path}/securics-${BUILD_TARGET}.spec ${rpm_build_dir}/SPECS/${package_name}.spec
...
build_package(){
    ...
    $linux $rpmbuild --define "_sysconfdir /etc" --define "_topdir ${rpm_build_dir}" \
        ...
        --define "_version ${securics_version}" --define "_hashcommit ${short_commit_hash}" \
...
get_package_and_checksum(){
    ...
    if [[ "${checksum}" == "yes" ]]; then
        cd "${rpm_build_dir}/RPMS" && sha512sum $RPM_NAME > /var/local/securics/$RPM_NAME.sha512
        if [[ "${src}" == "yes" ]]; then
            cd "${rpm_build_dir}/SRPMS" && sha512sum $SRC_NAME > /var/local/securics/$SRC_NAME.sha512
        fi
    fi

    if [[ "${src}" == "yes" ]]; then
        mv ${rpm_build_dir}/SRPMS/$SRC_NAME /var/local/securics
    else
        mv ${rpm_build_dir}/RPMS/$RPM_NAME /var/local/securics
    fi
}
